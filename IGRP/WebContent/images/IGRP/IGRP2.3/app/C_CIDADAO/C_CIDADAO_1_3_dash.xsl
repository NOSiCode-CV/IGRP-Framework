        ...
        <div class="IGRP_overlay"/><!--HOME TOP--><xsl:call-template name="home-top-main"/><!--HOME--><div id="igrp-bodyPage"><!--HOME LEFT--><xsl:if test="not(rows/target) or rows/target!='_blank'"><!--MENU--><div class="bodyPageLeft">
              ...
              <xsl:apply-templates mode="form-hidden" select="rows/content/form/value"/><!-- START FILTER--><div class="box-content resetPadding">
                <div class="col">
                  <xsl:if test="rows/content/filter/label/nome">
                    <div class="col-1-4 item" item-name="nome">
                      <div class="igrp_item text">
                        ...
                        <input type="text" name="{rows/content/filter/value/nome/@name}" value="{rows/content/filter/value/nome}" class="text required" maxlength="40">
                  ...
                  <xsl:if test="rows/content/filter/label/n_banco">
                    <div class="col-1-4 item" item-name="n_banco">
                      <div class="igrp_item select">
                        <label>
                          <span class="riquired">*</span>
                          <xsl:value-of select="rows/content/filter/label/n_banco" disable-output-escaping="yes"/>
                        </label>
                        <select name="{rows/content/filter/list/n_banco/@name}" chosen="select" data-placeholder="{rows/content/filter/list/n_banco/option[position() = 1]}" class="select required">
                          <xsl:call-template name="FIELD_validator">
                            <xsl:with-param name="field" select="rows/content/filter/label/n_banco"/>
                          </xsl:call-template>
                          <option value=""/>
                          <xsl:for-each select="rows/content/filter/list/n_banco/option[position() != 1]">
                            <option value="{value}">
                    ...
                    <table class="IGRP_table default-table" data-control="table">
                      <thead>
                        <tr>
                          ...
                          <xsl:if test="rows/content/table/label/n_banco">
                            <th align="center">
                              <xsl:value-of select="rows/content/table/label/n_banco"/>
                            </th>
                          </xsl:if>
                      ...
                      <tbody>
                        <xsl:for-each select="rows/content/table/value/row[not(@total='yes')]">
                          <tr row="{position()}" class="IGRP_contextMenu">
                            ...
                            <xsl:if test="morada">
                              <td align="center" data-row="{position()}" data-title="{../../label/morada}">
                                <xsl:value-of select="morada"/>
                              </td>
                            </xsl:if>
                            <xsl:if test="n_banco">
                              <td align="center" data-row="{position()}" data-title="{../../label/n_banco}">
                                <xsl:value-of select="n_banco"/>
                              </td>
                            </xsl:if>
                            <xsl:if test="nif">
                              <td align="left" data-row="{position()}" data-title="{../../label/nif}">
