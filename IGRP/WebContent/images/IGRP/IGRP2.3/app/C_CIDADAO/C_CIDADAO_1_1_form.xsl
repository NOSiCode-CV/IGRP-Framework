            ...
            <form action="#" method="post" id="formular_default" name="formular_default" class="default_form" enctype="multipart/form-data"><!-- SHOW HIDDEN--><xsl:apply-templates mode="form-hidden" select="rows/content/filter/value"/>
              <xsl:apply-templates mode="form-hidden" select="rows/content/form/value"/><!-- START FORM--><!--START TOOLS BAR FROM--><xsl:apply-templates mode="tools-bar" select="rows/content/form/tools-bar"/><!--END TOOLS BAR FORM--><div class="col addMarginTop" sep-name="form">
                  ...
                  <xsl:if test="rows/content/form/label/hora">
                    <div class="col-1-4 item" item-name="hora">
                      <div class="igrp_item text">
                        ...
                        <input type="text" name="{rows/content/form/value/hora/@name}" value="{rows/content/form/value/hora}" class="text" maxlength="8">
                          <xsl:call-template name="FIELD_validator">
                ...
                <xsl:if test="rows/content/form/label/dia">
                  <xsl:if test="rows/content/form/label/dia">
                    <div class="col-1-4 item" item-name="dia">
                      <div class="igrp_item select">
                        <label>
                          <xsl:value-of select="rows/content/form/label/dia" disable-output-escaping="yes"/>
                        </label>
                        <select name="{rows/content/form/list/dia/@name}" chosen="select" data-placeholder="{rows/content/form/list/dia/option[position() = 1]}" class="select">
                          <xsl:call-template name="FIELD_validator">
                            <xsl:with-param name="field" select="rows/content/form/label/dia"/>
                          </xsl:call-template>
                          <option value=""/>
                          <xsl:for-each select="rows/content/form/list/dia/option[position() != 1]">
                            <option value="{value}">
