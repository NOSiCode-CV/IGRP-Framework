                    ...
                    <div class="box-body" first-type-container="" last-type-container="">
                      <div role="form" gen-id="drop-zone">
                        ...
                        <xsl:if test="rows/content/filter/label/nome">
                          <div class="form-group col-md-3" item-name="nome" item-type="text" maxlength="50">
                            <xsl:call-template name="igrp-fields-validation">
                            ...
                            <input type="text" value="{rows/content/filter/value/nome}" class="form-control " id="{rows/content/filter/value/nome/@name}" name="{rows/content/filter/value/nome/@name}" maxlength="50">
                              <xsl:call-template name="igrp-fields-validation">
                                <xsl:with-param name="field" select="rows/content/filter/label/nome"/>
                              </xsl:call-template>
                            </input>
                      ...
                      <table id="table" class="table table-striped gen-data-table IGRP_contextmenu ">
                        <thead>
                          <tr>
                            ...
                            <xsl:if test="rows/content/table/label/user_insert">
                              <xsl:if test="not(rows/content/table/label/user_insert/@visible)">
                                <th item-name="user_insert" item-type="text">
                                  <xsl:value-of select="rows/content/table/label/user_insert"/>
                                </th>
                              </xsl:if>
                            </xsl:if>
                            <xsl:if test="rows/content/table/label/data_insert">
                              <xsl:if test="not(rows/content/table/label/data_insert/@visible)">
                                <th item-name="data_insert" item-type="date">
                                  <xsl:value-of select="rows/content/table/label/data_insert"/>
                                </th>
                              </xsl:if>
                            </xsl:if>
                            <xsl:if test="rows/content/table/label/imagem">
                              <xsl:if test="not(rows/content/table/label/imagem/@visible)">
                                <th item-name="imagem" item-type="link">
                                  <xsl:value-of select="rows/content/table/label/imagem"/>
                                </th>
                              </xsl:if>
                            </xsl:if>
                          </tr>
                        ...
                        <tbody>
                          <xsl:for-each select="rows/content/table/value/row[not(@total='yes')]">
                            <tr>
                              ...
                              <xsl:if test="user_insert">
                                <xsl:if test="not(user_insert/@visible)">
                                  <td item-name="user_insert" item-type="text" data-row="{position()}" data-title="{../../label/user_insert}">
                                    <span>
                                      <xsl:value-of select="user_insert"/>
                                    </span>
                                  </td>
                                </xsl:if>
                                <xsl:if test="user_insert/@visible = 'false'">
                                  <input type="hidden" name="p_user_insert_fk" value="user_insert"/>
                                </xsl:if>
                              </xsl:if>
                              <xsl:if test="data_insert">
                                <xsl:if test="not(data_insert/@visible)">
                                  <td item-name="data_insert" item-type="date" data-row="{position()}" data-title="{../../label/data_insert}">
                                    <span>
                                      <xsl:value-of select="data_insert"/>
                                    </span>
                                  </td>
                                </xsl:if>
                                <xsl:if test="data_insert/@visible = 'false'">
                                  <input type="hidden" name="p_data_insert_fk" value="data_insert"/>
                                </xsl:if>
                              </xsl:if>
                              <xsl:if test="imagem">
                                <xsl:if test="not(imagem/@visible)">
                                  <td item-name="imagem" item-type="link" data-row="{position()}" data-title="{../../label/imagem}">
                                    <xsl:choose>
                                      <xsl:when test="imagem != ''">
                                        <a href="{imagem}" class="link bClick" name="imagem" target="_blank">
                              ...
                              <input type="hidden" name="p_link_fk" value="{link}"/>
                              <input type="hidden" name="p_link_fk_desc" value="{link_desc}"/>
                              <input type="hidden" name="p_user_upadate_fk" value="{user_upadate}"/>
                              <input type="hidden" name="p_user_upadate_fk_desc" value="{user_upadate_desc}"/>
