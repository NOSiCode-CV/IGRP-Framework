<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
                ...
                <div class="col">
                  ...
                  <xsl:if test="rows/content/filter/label/description">
                    <div class="col-1-4 item" item-name="description">
                      <div class="igrp_item text">
                        <label>
                          <xsl:if test="string-length(rows/content/filter/label/description) > 45">
                            <xsl:attribute name="title">
                  ...
                  <xsl:if test="rows/content/filter/label/cod_lang">
                    <div class="col-1-4 item" item-name="cod_lang">
                      <div class="igrp_item select">
                        <label>
                          <xsl:if test="string-length(rows/content/filter/label/cod_lang) > 45">
                            <xsl:attribute name="title">
                              <xsl:value-of select="rows/content/filter/label/cod_lang" disable-output-escaping="yes"/>
                            </xsl:attribute>
                          </xsl:if>
                          <xsl:value-of select="rows/content/filter/label/cod_lang" disable-output-escaping="yes"/>
                        </label>
                        <select name="{rows/content/filter/list/cod_lang/@name}" chosen="select" id="{rows/content/filter/list/cod_lang/@name}" data-placeholder="{rows/content/filter/list/cod_lang/option[position() = 1]}" class="select">
                          <xsl:call-template name="FIELD_validator">
                            <xsl:with-param name="field" select="rows/content/filter/label/cod_lang"/>
                          </xsl:call-template>
                          <option value=""/>
                          <xsl:for-each select="rows/content/filter/list/cod_lang/option[position() != 1]">
                            <option value="{value}">
                    ...
                    <table class="IGRP_table default-table" data-control="table">
                      <thead>
                        <tr>
                          ...
                          <xsl:if test="rows/content/table/label/cod_lang">
                            <th align="left">
                              <xsl:value-of select="rows/content/table/label/cod_lang"/>
                            </th>
                          </xsl:if>
                        </tr>
                      </thead>
                      <tbody>
                        <xsl:for-each select="rows/content/table/value/row[not(@total='yes')]">
                          <tr row="{position()}" class="IGRP_contextMenu">
                            ...
                            <xsl:if test="cod_lang">
                              <td align="left" data-row="{position()}" data-title="{../../label/cod_lang}" class="text">
                                <xsl:value-of select="cod_lang"/>
                              </td>
                            </xsl:if>
                          </tr>
                        </xsl:for-each>
  ...
  <xsl:include href="../../xsl/tmpl/IGRP-context.tmpl.xsl?v=20150615"/>
</xsl:stylesheet>
