              ...
              <xsl:apply-templates mode="form-hidden" select="rows/content/form/value"/><!-- START FILTER--><!-- END FILTER--><!-- START TABELA--><div class="col addBorderLeftRight borderColor"><!--START TOOL BAR TABLE--><xsl:apply-templates mode="tools-bar" select="rows/content/table/tools-bar">
                  ...
                  <div class="table-container">
                    <table class="IGRP_table default-table" data-control="table">
                      <thead>
                        <tr>
                          <xsl:if test="rows/content/table/label/menu_fk">
                            <th class="checkbox sorttable_nosort" align="center">
                              <input type="checkbox" name="p_menu_fk_all" class="IGRP_checkall"/>
                            </th>
                          </xsl:if>
                          <xsl:if test="rows/content/table/label/menu">
                            <th align="left">
                              <xsl:value-of select="rows/content/table/label/menu"/>
                            </th>
                          </xsl:if>
                        </tr>
                      </thead>
                      <tbody>
                        <xsl:for-each select="rows/content/table/value/row[not(@total='yes')]">
                          <tr row="{position()}" class="">
                            ...
                            <xsl:if test="menu_fk">
                              <td align="center" data-row="{position()}" data-title="{../../label/menu_fk}" class="checkbox">
                                <xsl:if test="menu_fk != '-0'">
                                  <input type="checkbox" name="p_menu_fk" value="{menu_fk}">
                                    <xsl:if test="menu_fk_check=menu_fk">
                                      <xsl:attribute name="checked">checked</xsl:attribute>
                                    </xsl:if>
                                  </input>
                                </xsl:if>
                                <xsl:value-of select="menu_fk_desc"/>
                              </td>
                            </xsl:if>
                            <xsl:if test="menu">
                              <td align="left" data-row="{position()}" data-title="{../../label/menu}" class="text">
