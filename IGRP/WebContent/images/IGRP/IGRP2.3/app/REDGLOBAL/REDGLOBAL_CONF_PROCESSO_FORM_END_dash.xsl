        ...
        <div class="IGRP_overlay"/><!--HOME TOP--><xsl:call-template name="home-top-main"/><!--HOME--><div id="igrp-bodyPage"><!--HOME LEFT--><xsl:if test="not(rows/target) or rows/target!='_blank'"><!--MENU--><div class="bodyPageLeft">
            ...
            <form action="#" method="post" id="formular_default" name="formular_default" class="default_form" enctype="multipart/form-data"><!-- SHOW HIDDEN--><xsl:apply-templates mode="form-hidden" select="rows/content/filter/value"/>
              <xsl:apply-templates mode="form-hidden" select="rows/content/form/value"/><!-- START FORM--><!--START TOOLS BAR FROM--><xsl:apply-templates mode="tools-bar" select="rows/content/form/tools-bar"/><!--END TOOLS BAR FORM--><div class="col addMarginTop" sep-name="form">
                ...
                </xsl:if><!-- INICIO SEPARADOR sep_class_org--><xsl:if test="rows/content/form/label/sep_class_org">
                  <div class="box-content resetPadding" sep-name="sep_class_org">
                    <xsl:call-template name="titlecollapser">
                      <xsl:with-param name="title" select="rows/content/form/label/sep_class_org"/>
                    </xsl:call-template>
                    <div class="box-collapser">
                      <div class="_clear"/>
                    </div>
                ...
                <xsl:if test="rows/content/form/label/acti_tp_form_desc">
                  <xsl:if test="rows/content/form/label/acti_tp_form_desc">
                    <div class="col-1-4 item" item-name="acti_tp_form_desc">
                      <div class="igrp_item text">
                        <label>
                          <xsl:value-of select="rows/content/form/label/acti_tp_form_desc" disable-output-escaping="yes"/>
                        </label>
                        <input type="text" name="{rows/content/form/value/acti_tp_form_desc/@name}" value="{rows/content/form/value/acti_tp_form_desc}" class="text" maxlength="150">
                          <xsl:call-template name="FIELD_validator">
                            <xsl:with-param name="field" select="rows/content/form/label/acti_tp_form_desc"/>
                          </xsl:call-template>
                        </input>
                        <div class="_clear"/>
                      </div>
                ...
                </xsl:if><!-- INICIO SEPARADOR origens--><xsl:if test="rows/content/form/label/origens">
                  <div class="box-content resetPadding" list-name="origens">
                    <xsl:call-template name="titlecollapser">
                      <xsl:with-param name="title" select="rows/content/form/label/origens"/>
                    </xsl:call-template>
                    <div class="box-collapser"><!-- START TABELA LIST--><div class="box-table table-responsive">
                        <table rel="T_origens" data-control="data-origens">
                          <thead>
                            <tr>
                              <xsl:if test="rows/content/form/table/origens/label/etapa_origem">
                                <xsl:if test="not(rows/content/form/table/origens/label/etapa_origem/@visible)">
                                  <th>
                                    <xsl:value-of select="rows/content/form/table/origens/label/etapa_origem" disable-output-escaping="yes"/>
                                  </th>
                                </xsl:if>
                              </xsl:if>
                            </tr>
                          ...
                          <tbody>
                            <xsl:for-each select="rows/content/form/table/origens/value/row[not(@total='yes')]">
                              <tr row="{position()}">
                                <xsl:if test="etapa_origem">
                                  <xsl:if test="not(etapa_origem/@visible)">
                                    <td data-row="{position()}" data-title="{../../label/etapa_origem}" class="text">
                                      <xsl:value-of select="etapa_origem" disable-output-escaping="yes"/>
                                      <input type="hidden" name="p_etapa_origem_fk" value="{etapa_origem}"/>
                                      <span class="showResponsiveTable ctxmenu"/>
                                    </td>
                                  </xsl:if>
                                  <xsl:if test="etapa_origem/@visible = 'false'">
                                    <input type="hidden" name="p_etapa_origem_fk" value="{etapa_origem}"/>
                                  </xsl:if>
                                </xsl:if>
                                <input type="hidden" name="p_origens_id" value="{origens_id}"/>
                              </tr>
                            </xsl:for-each>
                          </tbody>
                        </table>
