<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
                ...
                <div class="col">
                  <xsl:if test="rows/content/filter/label/env_fk">
                    <div class="col-1-4 item" item-name="env_fk">
                      <div class="igrp_item selectchange">
                        ...
                        <select name="{rows/content/filter/list/env_fk/@name}" chosen="select" id="{rows/content/filter/list/env_fk/@name}" data-placeholder="{rows/content/filter/list/env_fk/option[position() = 1]}" class="selectchange IGRP_change">
                  ...
                  <xsl:if test="rows/content/filter/label/org_fk">
                    <div class="col-1-4 item" item-name="org_fk">
                      <div class="igrp_item selectchange">
                        ...
                        <select name="{rows/content/filter/list/org_fk/@name}" chosen="select" id="{rows/content/filter/list/org_fk/@name}" data-placeholder="{rows/content/filter/list/org_fk/option[position() = 1]}" class="selectchange IGRP_change">
                          <xsl:call-template name="FIELD_validator">
                  ...
                  <xsl:if test="rows/content/filter/label/self_id">
                    <div class="col-1-4 item" item-name="self_id">
                      <div class="igrp_item select">
                        <label>
                          <xsl:if test="string-length(rows/content/filter/label/self_id) > 45">
                            <xsl:attribute name="title">
                              <xsl:value-of select="rows/content/filter/label/self_id" disable-output-escaping="yes"/>
                            </xsl:attribute>
                          </xsl:if>
                          <xsl:value-of select="rows/content/filter/label/self_id" disable-output-escaping="yes"/>
                        </label>
                        <select name="{rows/content/filter/list/self_id/@name}" chosen="select" id="{rows/content/filter/list/self_id/@name}" data-placeholder="{rows/content/filter/list/self_id/option[position() = 1]}" class="select">
                          <xsl:call-template name="FIELD_validator">
                            <xsl:with-param name="field" select="rows/content/filter/label/self_id"/>
                          </xsl:call-template>
                          <option value=""/>
                          <xsl:for-each select="rows/content/filter/list/self_id/option[position() != 1]">
                            <option value="{value}">
                              <xsl:if test="@selected='true'">
                                <xsl:attribute name="selected">selected</xsl:attribute>
                              </xsl:if>
                  ...
                  <div class="table-container"><!--  CONTEXT-MENU  --><xsl:apply-templates mode="context-menu" select="rows/content/table/context-menu">
                    ...
                    <table class="IGRP_table default-table" data-control="table">
                      <thead>
                        <tr>
                          <xsl:if test="rows/content/table/label/self_id">
                            <th align="left">
                              <xsl:value-of select="rows/content/table/label/self_id"/>
                            </th>
                          </xsl:if>
                          ...
                          <xsl:if test="rows/content/table/label/action_fk">
                            <th align="left">
                              <xsl:value-of select="rows/content/table/label/action_fk"/>
                            </th>
                          </xsl:if>
                          ...
                          <xsl:if test="rows/content/table/label/flg_base">
                            <th class="checkbox sorttable_nosort" align="center">
                              <input type="checkbox" name="p_flg_base_all" class="IGRP_checkall"/>
                            </th>
                          </xsl:if>
                        </tr>
                      </thead>
                      <tbody>
                        <xsl:for-each select="rows/content/table/value/row[not(@total='yes')]">
                          <tr row="{position()}" class="IGRP_contextMenu">
                            ...
                            <xsl:if test="self_id">
                              <td align="left" data-row="{position()}" data-title="{../../label/self_id}" class="text">
                            ...
                            <xsl:if test="action_fk">
                              <td align="left" data-row="{position()}" data-title="{../../label/action_fk}" class="text">
                                <xsl:value-of select="action_fk"/>
                              </td>
                            </xsl:if>
                            <xsl:if test="status">
                            ...
                            <xsl:if test="flg_base">
                              <td align="left" data-row="{position()}" data-title="{../../label/flg_base}" class="checkbox">
                                <xsl:if test="flg_base != '-0'">
                                  <input type="checkbox" name="p_flg_base" value="{flg_base}">
                                    <xsl:if test="flg_base_check=flg_base">
                                      <xsl:attribute name="checked">checked</xsl:attribute>
                                    </xsl:if>
                                  </input>
                                </xsl:if>
                                <xsl:value-of select="flg_base_desc"/>
                              </td>
                            </xsl:if>
                          </tr>
                        </xsl:for-each>
  ...
  <xsl:include href="../../xsl/tmpl/IGRP-context.tmpl.xsl?v=20150821"/>
</xsl:stylesheet>
