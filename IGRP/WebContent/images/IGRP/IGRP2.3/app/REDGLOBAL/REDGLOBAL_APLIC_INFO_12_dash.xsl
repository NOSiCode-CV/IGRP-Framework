<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
        ...
        <div class="IGRP_overlay"/><!--HOME TOP--><xsl:call-template name="home-top-main"/><!--HOME--><div id="igrp-bodyPage"><!--HOME LEFT--><xsl:if test="not(rows/target) or rows/target!='_blank'"><!--MENU--><div class="bodyPageLeft">
            ...
            <form action="#" method="post" id="formular_default" name="formular_default" class="default_form" enctype="multipart/form-data"><!-- SHOW HIDDEN--><xsl:apply-templates mode="form-hidden" select="rows/content/filter/value"/>
              <xsl:apply-templates mode="form-hidden" select="rows/content/form/value"/><!-- START FORM--><div class="col addMarginTop" sep-name="form">
                <xsl:if test="rows/content/form/label/help_file">
                  <div class="box-content resetPadding" sep-name="help_file">
                    <xsl:call-template name="iframe">
                      <xsl:with-param name="url" select="rows/content/form/value/help_file"/>
                      <xsl:with-param name="name" select="help_file"/>
                    </xsl:call-template>
                  </div>
                </xsl:if>
                <div class="_clear"/>
  ...
  <xsl:include href="../../xsl/tmpl/IGRP-others.tmpl.xsl?v=20150309"/>
