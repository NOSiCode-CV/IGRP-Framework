                ...
                <xsl:if test="rows/content/form/label/estado">
                  ...
                  <div class="box-content resetPadding" sep-name="fases">
                    ...
                    <div class="box-collapser">
                      <div class="col" sep-name="group-fases">
                        <xsl:if test="rows/content/form/label/f_id_tp_fase">
                          <div class="col-1-4 item" item-name="f_id_tp_fase">
                            <div class="igrp_item select">
                              <label>
                                <xsl:if test="string-length(rows/content/form/label/f_id_tp_fase) > 45">
                                  <xsl:attribute name="title">
                                    <xsl:value-of select="rows/content/form/label/f_id_tp_fase" disable-output-escaping="yes"/>
                                  </xsl:attribute>
                                </xsl:if>
                                <span class="riquired">*</span>
                                <xsl:value-of select="rows/content/form/label/f_id_tp_fase" disable-output-escaping="yes"/>
                              </label>
                              <select name="{rows/content/form/list/f_id_tp_fase/@name}" chosen="select" id="{rows/content/form/list/f_id_tp_fase/@name}" data-placeholder="{rows/content/form/list/f_id_tp_fase/option[position() = 1]}" class="select important" rel="F_fases">
                                <xsl:call-template name="FIELD_validator">
                                  <xsl:with-param name="field" select="rows/content/form/label/f_id_tp_fase"/>
                                </xsl:call-template>
                                <option value=""/>
                                <xsl:for-each select="rows/content/form/list/f_id_tp_fase/option[position() != 1]">
                                  <option value="{value}">
                                    <xsl:if test="@selected='true'">
                                      <xsl:attribute name="selected">selected</xsl:attribute>
                                    </xsl:if>
                        ...
                        <xsl:if test="rows/content/form/label/f_estado">
                          <div class="col-1-4 item" item-name="f_estado">
                            <div class="igrp_item select">
                              ...
                              <select name="{rows/content/form/list/f_estado/@name}" chosen="select" id="{rows/content/form/list/f_estado/@name}" data-placeholder="{rows/content/form/list/f_estado/option[position() = 1]}" class="select important" rel="F_fases">
                                <xsl:call-template name="FIELD_validator">
                        ...
                        <xsl:if test="rows/content/form/label/f_nu_ordem">
                          <div class="col-1-4 item" item-name="f_nu_ordem">
                            <div class="igrp_item text">
                              <label>
                                <xsl:if test="string-length(rows/content/form/label/f_nu_ordem) > 45">
                                  <xsl:attribute name="title">
                                    <xsl:value-of select="rows/content/form/label/f_nu_ordem" disable-output-escaping="yes"/>
                                  </xsl:attribute>
                                </xsl:if>
                                <span class="riquired">*</span>
                                <xsl:value-of select="rows/content/form/label/f_nu_ordem" disable-output-escaping="yes"/>
                              </label>
                              <input type="text" name="{rows/content/form/value/f_nu_ordem/@name}" value="{rows/content/form/value/f_nu_ordem}" class="text important" maxlength="30" rel="F_fases">
                                <xsl:call-template name="FIELD_validator">
                                  <xsl:with-param name="field" select="rows/content/form/label/f_nu_ordem"/>
                                </xsl:call-template>
                              </input>
                              <div class="_clear"/>
                            </div>
                      ...
                      <xsl:call-template name="tools-bar-form-group">
                        ...
                        <xsl:with-param name="rel" select="'fases'"/>
                      </xsl:call-template>
                      <div class="col" sep-name="list-fases"><!-- START TABELA LIST--><div class="box-table table-responsive">
                          <ul class="contextMenu ctxSepListDialog" id="data-fases" sep-name="btn-fases">
                            <li rel="fases" class="IGRP_editRow operationTable" item-name="edit-fases">
                            ...
                            <li rel="fases" class="IGRP_delRow operationTable" item-name="del-fases">
                          ...
                          <table rel="T_fases" data-control="data-fases">
                            <thead>
                              <tr>
                                <xsl:if test="rows/content/form/table/fases/label/f_id_tp_fase">
                                  <xsl:if test="not(rows/content/form/table/fases/label/f_id_tp_fase/@visible)">
                                    <th>
                                      <xsl:value-of select="rows/content/form/table/fases/label/f_id_tp_fase" disable-output-escaping="yes"/>
                                    </th>
                                  </xsl:if>
                                </xsl:if>
                                <xsl:if test="rows/content/form/table/fases/label/f_estado">
                                  <xsl:if test="not(rows/content/form/table/fases/label/f_estado/@visible)">
                                    <th>
                                      <xsl:value-of select="rows/content/form/table/fases/label/f_estado" disable-output-escaping="yes"/>
                                    </th>
                                  </xsl:if>
                                </xsl:if>
                                <xsl:if test="rows/content/form/table/fases/label/f_nu_ordem">
                                  <xsl:if test="not(rows/content/form/table/fases/label/f_nu_ordem/@visible)">
                                    <th>
                                      <xsl:value-of select="rows/content/form/table/fases/label/f_nu_ordem" disable-output-escaping="yes"/>
                                    </th>
                                  </xsl:if>
                                </xsl:if>
                                <th class="bEdDel"/>
                            ...
                            <tbody>
                              <xsl:for-each select="rows/content/form/table/fases/value/row[not(@total='yes')]">
                                <tr row="{position()}">
                                  <xsl:if test="f_id_tp_fase">
                                    <xsl:if test="not(f_id_tp_fase/@visible)">
                                      <td data-row="{position()}" data-title="{../../label/f_id_tp_fase}" class="select">
                                        <xsl:value-of select="f_id_tp_fase_desc" disable-output-escaping="yes"/>
                                        <input type="hidden" name="p_f_id_tp_fase_fk" value="{f_id_tp_fase}"/>
                                        <input type="hidden" name="p_f_id_tp_fase_fk_desc" value="{f_id_tp_fase_desc}"/>
                                        <span class="showResponsiveTable ctxmenu"/>
                                      </td>
                                    </xsl:if>
                                    <xsl:if test="f_id_tp_fase/@visible = 'false'">
                                      <input type="hidden" name="p_f_id_tp_fase_fk" value="{f_id_tp_fase}"/>
                                      <input type="hidden" name="p_f_id_tp_fase_fk_desc" value="{f_id_tp_fase_desc}"/>
                                    </xsl:if>
                                  </xsl:if>
                                  ...
                                  <xsl:if test="f_nu_ordem">
                                    <xsl:if test="not(f_nu_ordem/@visible)">
                                      <td data-row="{position()}" data-title="{../../label/f_nu_ordem}" class="text">
                                        <xsl:value-of select="f_nu_ordem_desc" disable-output-escaping="yes"/>
                                        <input type="hidden" name="p_f_nu_ordem_fk" value="{f_nu_ordem}"/>
                                        <input type="hidden" name="p_f_nu_ordem_fk_desc" value="{f_nu_ordem_desc}"/>
                                      </td>
                                    </xsl:if>
                                    <xsl:if test="f_nu_ordem/@visible = 'false'">
                                      <input type="hidden" name="p_f_nu_ordem_fk" value="{f_nu_ordem}"/>
                                      <input type="hidden" name="p_f_nu_ordem_fk_desc" value="{f_nu_ordem_desc}"/>
                                    </xsl:if>
                                  </xsl:if>
                                  <td class="bEdDel" data-row="{position()}" data-title="{$lngPath/separatorlist_dialog/edit}/{$lngPath/separatorlist_dialog/delete}">
                                    <xsl:if test="not(@noupdate)">
                                      <img title="{$lngPath/separatorlist_dialog/edit}" src="{$path_tmpl}/img/icon/tools-bar/edit.png" item-name="edit-fases" rel="fases" class="IGRP_editRow"/>
                                    </xsl:if>
                                    <xsl:if test="not(@nodelete)">
                                      <img title="{$lngPath/separatorlist_dialog/delete}" src="{$path_tmpl}/img/icon/tools-bar/delete.png" item-name="del-fases" rel="fases" class="IGRP_delRow"/>
                                    </xsl:if>
                                  </td>
