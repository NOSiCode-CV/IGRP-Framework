    ...
    <html><!--XSL Generator (RED/DIM 2015)--><head><!--HOME-HEADER--><xsl:call-template name="home-header"/>
      ...
      <body class="bodyColor">
        <div class="IGRP_overlay"/><!--HOME TOP--><xsl:call-template name="home-top-main"/><!--HOME--><div id="igrp-bodyPage"><!--HOME LEFT--><xsl:call-template name="home-slide-menu"/><!--HOME RIGTH--><div class="bodyPageRigth"><!-- START YOUR CODE HERE --><!-- SHOW TABLET AND MOBILE TITLE --><div class="box-head showTabMobile">
                    ...
                    <table class="IGRP_table default-table" data-control="table">
                      <thead>
                        <tr>
                          ...
                          <xsl:if test="rows/content/table/label/analise">
                            <th align="">
                              <xsl:value-of select="rows/content/table/label/analise"/>
                            </th>
                          </xsl:if>
                          <xsl:if test="rows/content/table/label/desenv">
                            <th align="">
                              <xsl:value-of select="rows/content/table/label/desenv"/>
                            </th>
                          </xsl:if>
                          ...
                          <xsl:if test="rows/content/table/label/deploy">
                            <th align="">
                              <xsl:value-of select="rows/content/table/label/deploy"/>
                            </th>
                          </xsl:if>
                          <xsl:if test="rows/content/table/label/design">
                            <th align="">
                              <xsl:value-of select="rows/content/table/label/design"/>
                            </th>
                          </xsl:if>
                      ...
                      <tbody>
                        <xsl:for-each select="rows/content/table/value/row[not(@total='yes')]">
                          <tr row="{position()}" class="">
                            ...
                            <xsl:if test="analise">
                              <td align="" data-row="{position()}" data-title="{../../label/analise}" class="number">
                                <xsl:value-of select="analise"/>
                              </td>
                            </xsl:if>
                            <xsl:if test="desenv">
                              <td align="" data-row="{position()}" data-title="{../../label/desenv}" class="number">
                                <xsl:value-of select="desenv"/>
                              </td>
                            </xsl:if>
                            <xsl:if test="teste">
                              <td align="" data-row="{position()}" data-title="{../../label/teste}" class="number">
                                <xsl:value-of select="teste"/>
                              </td>
                            </xsl:if>
                            <xsl:if test="deploy">
                              <td align="" data-row="{position()}" data-title="{../../label/deploy}" class="number">
                                <xsl:value-of select="deploy"/>
                              </td>
                            </xsl:if>
                            <xsl:if test="design">
                              <td align="" data-row="{position()}" data-title="{../../label/design}" class="number">
                                <xsl:value-of select="design"/>
                              </td>
                            </xsl:if>
                            <xsl:if test="total">
                              <td align="" data-row="{position()}" data-title="{../../label/total}" class="number">
            ...
            <div class="col">
              <xsl:if test="rows/content/total_proj_dep"><!-- START CHARTS--><div class="col-1-400" sep-name="total_proj_dep">
                  <xsl:call-template name="igrp-graph">
                    <xsl:with-param name="table" select="rows/content/total_proj_dep"/>
                    <xsl:with-param name="chart_type" select="rows/content/total_proj_dep/chart_type"/>
                    <xsl:with-param name="height" select="300"/>
                    <xsl:with-param name="title" select="rows/content/total_proj_dep/caption"/>
                  </xsl:call-template>
                </div><!-- END CHARTS--></xsl:if>
              <xsl:if test="rows/content/total_proj_dep_fase"><!-- START CHARTS--><div class="col-1-400" sep-name="total_proj_dep_fase">
                  <xsl:call-template name="igrp-graph">
                    <xsl:with-param name="table" select="rows/content/total_proj_dep_fase"/>
                    <xsl:with-param name="chart_type" select="rows/content/total_proj_dep_fase/chart_type"/>
                    <xsl:with-param name="height" select="300"/>
                    <xsl:with-param name="title" select="rows/content/total_proj_dep_fase/caption"/>
                  </xsl:call-template>
                </div><!-- END CHARTS--></xsl:if>
              <div class="_clear"/>
