<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
      ...
      <body class="bodyColor">
        <div class="IGRP_overlay"/><!--HOME TOP--><xsl:call-template name="home-top-main"/><!--HOME--><div id="igrp-bodyPage"><!--HOME LEFT--><xsl:call-template name="home-slide-menu"/><!--HOME RIGTH--><div class="bodyPageRigth"><!-- START YOUR CODE HERE --><!-- SHOW TABLET AND MOBILE TITLE --><div class="box-head showTabMobile">
              ...
              <xsl:apply-templates mode="form-hidden" select="rows/content/form/value"/><!-- START FORM--><!--START TOOLS BAR FROM--><xsl:apply-templates mode="tools-bar" select="rows/content/form/tools-bar"/><!--END TOOLS BAR FORM--><div class="col addMarginTop addPaddingBottom10" sep-name="form">
                <xsl:if test="rows/content/form/label/a6">
                  <div class="col-1-4 item" item-name="a6">
                    <div class="igrp_item text">
                      <label>
                        <xsl:if test="string-length(rows/content/form/label/a6) > 45">
                          <xsl:attribute name="title">
                            <xsl:value-of select="rows/content/form/label/a6" disable-output-escaping="yes"/>
                          </xsl:attribute>
                        </xsl:if>
                        <xsl:value-of select="rows/content/form/label/a6" disable-output-escaping="yes"/>
                      </label>
                      <input type="text" name="{rows/content/form/value/a6/@name}" value="{rows/content/form/value/a6}" class="text" maxlength="30">
                        <xsl:call-template name="FIELD_validator">
                          <xsl:with-param name="field" select="rows/content/form/label/a6"/>
                        </xsl:call-template>
                      </input>
                      <div class="_clear"/>
                    </div>
                ...
                <div class="col">
                  <xsl:if test="rows/content/filter/label/a2">
                    <div class="col-1-4 item" item-name="a2">
                      <div class="igrp_item text">
                        <label>
                          <xsl:if test="string-length(rows/content/filter/label/a2) > 45">
                            <xsl:attribute name="title">
                              <xsl:value-of select="rows/content/filter/label/a2" disable-output-escaping="yes"/>
                            </xsl:attribute>
                          </xsl:if>
                          <xsl:value-of select="rows/content/filter/label/a2" disable-output-escaping="yes"/>
                        </label>
                        <input type="text" name="{rows/content/filter/value/a2/@name}" value="{rows/content/filter/value/a2}" class="text" maxlength="30">
                          <xsl:call-template name="FIELD_validator">
                            <xsl:with-param name="field" select="rows/content/filter/label/a2"/>
                          </xsl:call-template>
                        </input>
                        <div class="_clear"/>
                      </div>
                  ...
                  <div class="table-container"><!--  CONTEXT-MENU  --><xsl:apply-templates mode="context-menu" select="rows/content/table/context-menu">
                    ...
                    <table class="IGRP_table default-table" data-control="table">
                      <thead>
                        <tr>
                          <xsl:if test="rows/content/table/label/a3">
                            <th align="">
                              <xsl:value-of select="rows/content/table/label/a3"/>
                            </th>
                          </xsl:if>
                        </tr>
                      </thead>
                      <tbody>
                        <xsl:for-each select="rows/content/table/value/row[not(@total='yes')]">
                          <tr row="{position()}" class="IGRP_contextMenu">
                            ...
                            <xsl:if test="a3">
                              <td align="" data-row="{position()}" data-title="{../../label/a3}" class="text">
            ...
            <div class="col">
              ...
              <div class="box-content resetPadding">
                <div class="col">
                  <xsl:if test="rows/content/a8"><!-- START CHARTS--><div class="col" sep-name="a8">
                      <xsl:call-template name="igrp-graph">
                        <xsl:with-param name="table" select="rows/content/a8"/>
                        <xsl:with-param name="chart_type" select="rows/content/a8/chart_type"/>
                        <xsl:with-param name="height" select="150"/>
                        <xsl:with-param name="title" select="rows/content/a8/caption"/>
                      </xsl:call-template>
  ...
  <xsl:include href="../../xsl/tmpl/IGRP-home-include.tmpl.xsl?v=20150716"/>
  <xsl:include href="../../xsl/tmpl/IGRP-setAttribute.tmpl.xsl?v=20150716"/>
  <xsl:include href="../../xsl/tmpl/IGRP-slide-menu.tmpl.xsl?v=20150716"/>
  <xsl:include href="../../xsl/tmpl/IGRP-view.tmpl.xsl?v=20150716"/>
  <xsl:include href="../../xsl/tmpl/IGRP-graph.tmpl.xsl?v=20150716"/>
  <xsl:include href="../../xsl/tmpl/IGRP-context.tmpl.xsl?v=20150716"/>
</xsl:stylesheet>
