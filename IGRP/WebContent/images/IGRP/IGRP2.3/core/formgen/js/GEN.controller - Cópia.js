...
var GENERATOR = function(genparams){
			...
			row.columns.forEach(function(column,cidx){
				...
				column.containers.forEach(function(_container){
					...
					if(container){
						...
						//if(isXML) _c += '<xsl:if xmlns:xsl="http://www.w3.org/1999/XSL/Transform" test="'+container.GET.path()+'">';
	...
	GEN.getSetter = function(p){
		...
		switch(type){
			//OPTIONS / comboboxx
			case 'select':
				var select = $('<select rel="'+propriety+'" class="form-control '+VARS.edition.class.propSetter+'"/>');
				...
				options.forEach(function(o){
					var selected = o.value == value ? ' selected="true"' : '';
					select.append('<option'+selected+' value="'+o.value+'">'+o.label+'</option>')
				});
	...
	var setHtmlStyle = function(o){
		if(o && o.customStyle){
			$('>.container-contents>.gen-container-item',o.holder).attr('id',o.customStyle.id);
			$('>.container-contents>.gen-container-item',o.holder).addClass(o.customStyle.class);
			$('>.container-contents>.gen-container-item',o.holder).attr('style',o.customStyle.inline);
		}
	}
	...
	var setEvents = function(){
		...
		$('#gen-edit-confirm').on('click',function(){
			...
			if(GEN.edit.object.formField){
				...
				var isTable = GEN.edit.object.parent.type == 'formlist' || GEN.edit.object.parent.type == 'separatorlist' ? true : false;
	...
	GEN.setBTNClass = function(field,_default){
		field.setPropriety({
			name:'class',
			value:{
		...
		if(!field.hidden){
			var sizeOptions = field.GET.type() == 'texteditor' || field.GET.type() == 'separator' ? 
			  ...
			  } :
			  {
			  	...
			  	options:[{value:'12',label:'100%'},{value:'9',label:'75%'},{value:'8',label:'66.66%'},{value:'6',label:'50%'},{value:'4',label:'33%'},{value:'3',label:'25%'},{value:'2',label:'16.6%'}]
			  }

			if(field.GET.type() != 'separator' && field.GET.type() != 'img'){

				if(field.GET.type() != 'link' && field.GET.type()!='plaintext'){
					field.setPropriety({
						name:'required',
						value    : false,
				...
				if(field.GET.type() != "plaintext" && field.GET.type() != "select" && field.GET.type() != "file" && field.GET.type() != "radio" && field.GET.type() != "checkbox" && field.GET.type() != "checkboxlist" && field.GET.type() != "radiolist"){
					field.setPropriety({
						name:'maxlength',
						propriety:30,
	...
	var getXSLTop = function(){
		...
		includes+='<style>'+GEN.cssEditor.getValue()+'</style>'
