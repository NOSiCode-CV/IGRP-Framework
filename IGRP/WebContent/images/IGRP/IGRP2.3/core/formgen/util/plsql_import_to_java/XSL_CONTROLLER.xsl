...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="create-controller">
    	<xsl:value-of select="$newline"/>
     	<xsl:call-template name="import-packages-controller"/>
     	<xsl:value-of select="$newline"/>
            ...
            <xsl:for-each select="/rows/content/*[@type = 'toolsbar' or @type='verticalmenu']/item">   <!-- Button in tools-bar -->
            	<xsl:if test="not(@rel=preceding::node()/@rel)">
		          	<xsl:call-template name="actions">
						...
						<xsl:with-param name="crud_operation"><xsl:value-of select="@crud_op"/></xsl:with-param>
		            </xsl:call-template>
		        </xsl:if>
           </xsl:for-each>
	...
	<xsl:template name="actions">
		...
		<xsl:param name="crud_operation" select="''"/>
		...
		<xsl:call-template name="gen-action">
			...
			<xsl:with-param name="crud_operation" select="$crud_operation"/>
		</xsl:call-template>
		<xsl:value-of select="$newline"/>
		<xsl:value-of select="$tab"/>
	</xsl:template>
	...
	<xsl:template name="gen-action">
		...
		<xsl:choose>
			<xsl:when test="$page_ != '' and $your_code='' and $crud_operation='' and $isCrud=''">
				<xsl:choose>
					<xsl:when test="$type_render_='render'">
						<xsl:call-template name="renderFunction">
							<xsl:with-param name="model" select="$model"/>
						</xsl:call-template>
					</xsl:when>
					<xsl:when test="$type_render_='redirect'">
						<xsl:call-template name="redirectFunction">
							<xsl:with-param name="page_" select="$page_"/>
							<xsl:with-param name="app__" select="$app__"/>
						</xsl:call-template>
					</xsl:when>					
					<xsl:when test="$type_render_='render_message'">
						<xsl:call-template name="renderContentFunction">
							<xsl:with-param name="model" select="$model"/>
						</xsl:call-template>
					</xsl:when>
					<xsl:otherwise>
			...
			<xsl:when test="$crud_operation!=''">
				<xsl:choose>
					<xsl:when test="$crud_operation='edit'">
						<xsl:call-template name="functionCRUDEdit">
							<xsl:with-param name="page_" select="$page_"/>
							<xsl:with-param name="app__" select="$app__"/>
						</xsl:call-template>
					</xsl:when>					
					<xsl:when test="$crud_operation='delete'">
						<xsl:call-template name="functionCRUDDelete">
							<xsl:with-param name="page_" select="$page_"/>
							<xsl:with-param name="app__" select="$app__"/>
						</xsl:call-template>
					</xsl:when>				
					<xsl:when test="$crud_operation='addNew'">
						<xsl:call-template name="functionCRUDAddNew">
							<xsl:with-param name="page_" select="$page_"/>
							<xsl:with-param name="app__" select="$app__"/>
						</xsl:call-template>
					</xsl:when>			
					<xsl:when test="$crud_operation='list'">
						<xsl:call-template name="functionCRUDList">
							<xsl:with-param name="page_" select="$page_"/>
							<xsl:with-param name="app__" select="$app__"/>
						</xsl:call-template>
					</xsl:when>		
					<xsl:when test="$crud_operation='save'">
						<xsl:call-template name="functionCRUDSave">
							<xsl:with-param name="page_" select="$page_"/>
							<xsl:with-param name="app__" select="$app__"/>
							<xsl:with-param name="primaryKey" select="$isCrud"/>
						</xsl:call-template>
					</xsl:when>
					<xsl:when test="$crud_operation='save'">
						<xsl:call-template name="functionCRUDSave">
					...
					<xsl:when test="$crud_operation='saveEdit'">						
						<xsl:call-template name="functionCRUDEditSave">
							<xsl:with-param name="page_" select="$page_"/>
							<xsl:with-param name="app__" select="$app__"/>
							<xsl:with-param name="primaryKey" select="$isCrud"/>
						</xsl:call-template>
			...
			<xsl:when test="$isCrud !=''">				
				<xsl:call-template name="functionCRUDIndex">
					<xsl:with-param name="model" select="$model"/>
					<xsl:with-param name="primaryKey" select="$isCrud"/>
				</xsl:call-template>	
			</xsl:when>
	...
	<xsl:template name="actionSaveEdit">
		<xsl:if test="/rows/content/@type = 'form'">
			<xsl:call-template name="gen-action">
				<xsl:with-param name="action_name_"><xsl:value-of select="'editar'"/></xsl:with-param>
				<xsl:with-param name="app_"><xsl:value-of select="$app_name"/></xsl:with-param>
				<xsl:with-param name="page_"><xsl:value-of select="$class_name"/></xsl:with-param>
				<xsl:with-param name="type_render_"><xsl:value-of select="'render'"/></xsl:with-param>
				<xsl:with-param name="crud_operation" select="'saveEdit'"/>
			</xsl:call-template>
		</xsl:if>
 	</xsl:template>
	...
	<xsl:template name="setParam">
 	...
 	<xsl:template name="renderFunction">
 	...
 	<xsl:template name="redirectFunction">
 		<xsl:param name="page_"/>
 		<xsl:param name="app__"/>
 	...
 	<xsl:template name="renderContentFunction">
 		<xsl:param name="model"/>
 	...
 	<xsl:template name="functionCRUDAddNew">
 		<xsl:param name="page_"/>
 		<xsl:param name="app__"/>
 		
 		<xsl:value-of select="$newline"/>
 		<xsl:value-of select="$tab2"/>
		<xsl:value-of select="concat('return this.redirect(',$double_quotes,$app__,$double_quotes,',',$double_quotes,$page_,$double_quotes,',',$double_quotes,'index&amp;target=_blank',$double_quotes,');')"/>
		<xsl:value-of select="$newline"/>  
		<xsl:value-of select="$tab"/>
 	</xsl:template>
 	
 	<xsl:template name="functionCRUDList">
 		<xsl:param name="page_"/>
 		<xsl:param name="app__"/>
 	...
 	<xsl:template name="functionCRUDSave">
 		<xsl:param name="page_"/>
 		<xsl:param name="app__"/>
		...
		<xsl:value-of select="$tab2"/>		
		<xsl:value-of select="$tab"/>	
		<xsl:value-of select="concat('String id = Igrp.getInstance().getRequest().getParameter(',$double_quotes,$primaryKey,$double_quotes,');')"/>
		<xsl:value-of select="$newline"/>
		<xsl:value-of select="$tab2"/>		
		<xsl:value-of select="$tab"/>
		<xsl:value-of select="concat('','if((',$primaryKey,'==null || !',$primaryKey,'.equals(',$double_quotes,$double_quotes,')) &amp;&amp; model.save(model)){')"/>
		<xsl:value-of select="$newline"/>
		<xsl:value-of select="$tab2"/>		
		<xsl:value-of select="$tab2"/>
 	...
 	<xsl:template name="functionCRUDEditSave">
 		<xsl:param name="page_"/>
 		<xsl:param name="app__"/>
 		<xsl:param name="primaryKey"/>
 		...
 		<xsl:if test="/rows/content/@type = 'form'">
			...
			<xsl:value-of select="$tab2"/>		
			<xsl:value-of select="$tab"/>
			<xsl:value-of select="concat('','if(',$primaryKey,'!=null &amp;&amp; model.update(model',',',$primaryKey,')){')"/>
			<xsl:value-of select="$newline"/>
			<xsl:value-of select="$tab2"/>		
			<xsl:value-of select="$tab2"/>
			<xsl:value-of select="'Core.setMessageSuccess();'"/>
 	...
 	<xsl:template name="functionCRUDEdit">
 		<xsl:param name="page_"/>
 		<xsl:param name="app__"/>
		...
		<xsl:value-of select="$newline"/>
		<xsl:value-of select="$tab2"/>
		<xsl:value-of select="concat('return this.redirect(',$double_quotes,$app__,$double_quotes,',',$double_quotes,$page_,$double_quotes,',',$double_quotes,'index&amp;target=_blank&amp;id=',$double_quotes,'+id',');')"/>
		<xsl:value-of select="$newline"/>  
		<xsl:value-of select="$tab"/>
 	</xsl:template>
 	
 	<xsl:template name="functionCRUDIndex">
 		<xsl:param name="model"/>
 		<xsl:param name="primaryKey"/>
		...
		<xsl:value-of select="$tab2"/>
		<xsl:if test="/rows/content/@type = 'form'">			
			<xsl:value-of select="concat('if(',$primaryKey,'!=null &amp;&amp; !',$primaryKey,'.equals(',$double_quotes,$double_quotes,')) {')"/>
			<xsl:value-of select="$newline"/>
			<xsl:value-of select="$tab2"/>
			<xsl:value-of select="$tab"/>
			<xsl:value-of select="concat('view.','btn_gravar','.setLink(',$double_quotes,'Editar&amp;',$primaryKey,'=',$double_quotes,'+',$primaryKey,');')"/>
			<xsl:value-of select="$newline"/>
			<xsl:value-of select="$tab2"/>
			<xsl:value-of select="'}'"/>
			<xsl:value-of select="$newline"/>
 	...
 	<xsl:template name="functionCRUDDelete">
 		<xsl:param name="page_"/>
 		<xsl:param name="app__"/>
		...
		<xsl:value-of select="$newline"/>
		<xsl:value-of select="$tab2"/>							
		<xsl:value-of select="concat('','if(id!=null){')"/>
		<xsl:value-of select="$newline"/>
		<xsl:value-of select="$tab2"/>		
		<xsl:value-of select="$tab"/>
		<xsl:value-of select="concat('','if(model.delete(id)){')"/>
		<xsl:value-of select="$newline"/>
		<xsl:value-of select="$tab2"/>		
		<xsl:value-of select="$tab2"/>
		<xsl:value-of select="'Core.setMessageSuccess();'"/>
