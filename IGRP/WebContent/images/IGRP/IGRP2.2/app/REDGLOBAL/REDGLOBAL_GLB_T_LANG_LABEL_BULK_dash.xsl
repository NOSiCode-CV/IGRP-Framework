<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
        ...
        <div class="IGRP_overlay"/><!--HOME TOP--><xsl:call-template name="home-top-main"/><!--HOME--><div id="igrp-bodyPage"><!--HOME LEFT--><xsl:call-template name="home-slide-menu"/><!--HOME RIGTH--><div class="bodyPageRigth"><!-- START YOUR CODE HERE --><!-- SHOW TABLET AND MOBILE TITLE --><div class="box-head showTabMobile">
              ...
              <xsl:apply-templates mode="form-hidden" select="rows/content/form/value"/><!-- START FILTER--><div class="box-content resetPadding">
                <div class="col">
                  <xsl:if test="rows/content/filter/label/code_lang">
                    <div class="col-1-4 item" item-name="code_lang">
                      <div class="igrp_item select">
                        <label>
                          <xsl:if test="string-length(rows/content/filter/label/code_lang) > 45">
                            <xsl:attribute name="title">
                              <xsl:value-of select="rows/content/filter/label/code_lang" disable-output-escaping="yes"/>
                            </xsl:attribute>
                          </xsl:if>
                          <xsl:value-of select="rows/content/filter/label/code_lang" disable-output-escaping="yes"/>
                        </label>
                        <select name="{rows/content/filter/list/code_lang/@name}" chosen="select" id="{rows/content/filter/list/code_lang/@name}" data-placeholder="{rows/content/filter/list/code_lang/option[position() = 1]}" class="select">
                          <xsl:call-template name="FIELD_validator">
                            <xsl:with-param name="field" select="rows/content/filter/label/code_lang"/>
                          </xsl:call-template>
                          <option value=""/>
                          <xsl:for-each select="rows/content/filter/list/code_lang/option[position() != 1]">
                            <option value="{value}">
                  ...
                  <xsl:if test="rows/content/filter/label/label">
                    <div class="col-1-4 item" item-name="label">
                      <div class="igrp_item text">
                        <label>
                          <xsl:if test="string-length(rows/content/filter/label/label) > 45">
                            <xsl:attribute name="title">
                              <xsl:value-of select="rows/content/filter/label/label" disable-output-escaping="yes"/>
                            </xsl:attribute>
                          </xsl:if>
                          <xsl:value-of select="rows/content/filter/label/label" disable-output-escaping="yes"/>
                        </label>
                        <input type="text" name="{rows/content/filter/value/label/@name}" value="{rows/content/filter/value/label}" class="text" maxlength="250">
                          <xsl:call-template name="FIELD_validator">
                            <xsl:with-param name="field" select="rows/content/filter/label/label"/>
                          </xsl:call-template>
                        </input>
                        <div class="_clear"/>
                      </div>
                      ...
                      <tbody>
                        <xsl:for-each select="rows/content/table/value/row[not(@total='yes')]">
                          <tr row="{position()}" class="IGRP_contextMenu">
                            ...
                            <xsl:if test="label">
                              <td align="left" data-row="{position()}" data-title="{../../label/label}" class="text">
                                <xsl:value-of select="label"/>
                              </td>
                            </xsl:if>
                            <xsl:if test="code_lang">
                              <td align="left" data-row="{position()}" data-title="{../../label/code_lang}" class="text">
  ...
  <xsl:include href="../../xsl/tmpl/IGRP-context.tmpl.xsl?v=20150617"/>
</xsl:stylesheet>
