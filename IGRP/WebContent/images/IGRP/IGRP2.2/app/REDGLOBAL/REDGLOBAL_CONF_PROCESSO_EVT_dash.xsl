        ...
        <div class="IGRP_overlay"/><!--HOME TOP--><xsl:call-template name="home-top-main"/><!--HOME--><div id="igrp-bodyPage"><!--HOME LEFT--><xsl:if test="not(rows/target) or rows/target!='_blank'"><!--MENU--><div class="bodyPageLeft">
            ...
            <form action="#" method="post" id="formular_default" name="formular_default" class="default_form" enctype="multipart/form-data"><!-- SHOW HIDDEN--><xsl:apply-templates mode="form-hidden" select="rows/content/filter/value"/>
              <xsl:apply-templates mode="form-hidden" select="rows/content/form/value"/><!-- START FORM--><!--START TOOLS BAR FROM--><xsl:apply-templates mode="tools-bar" select="rows/content/form/tools-bar"/><!--END TOOLS BAR FORM--><div class="col addMarginTop" sep-name="form">
                ...
                </xsl:if><!-- INICIO SEPARADOR sep_class_dest--><xsl:if test="rows/content/form/label/sep_class_dest">
                  <div class="box-content resetPadding" sep-name="sep_class_dest">
                    <xsl:call-template name="titlecollapser">
                      <xsl:with-param name="title" select="rows/content/form/label/sep_class_dest"/>
                    </xsl:call-template>
                    <div class="box-collapser">
                      <div class="_clear"/>
                    </div>
                ...
                </xsl:if><!-- INICIO SEPARADOR destino--><xsl:if test="rows/content/form/label/destino">
                  <div class="box-content resetPadding" sep-name="destino">
                    <xsl:call-template name="titlecollapser">
                ...
                <xsl:if test="rows/content/form/label/criar_acti">
                  <xsl:if test="rows/content/form/label/criar_acti">
                    <div class="col-1-4 item" item-name="criar_acti">
                      <div class="igrp_item radiolist">
                        <a id="idp_criar_acti"/>
                        <label>
                          <xsl:value-of select="rows/content/form/label/criar_acti" disable-output-escaping="yes"/>
                        </label>
                        <xsl:for-each select="rows/content/form/list/criar_acti/option">
                          <div class="col-1-1">
                            <input type="radio" name="{../@name}" value="{value}" class="radiolist IGRP_change">
                              <xsl:call-template name="FIELD_validator">
                                <xsl:with-param name="field" select="../../../label/criar_acti"/>
                              </xsl:call-template>
                              <xsl:if test="@selected='true'">
                ...
                <xsl:if test="rows/content/form/label/new_class">
                  <xsl:if test="rows/content/form/label/new_class">
                    <div class="col-1-4 item" item-name="new_class">
                      <div class="igrp_item select">
                        <a id="idp_new_class"/>
                        <label>
                          <xsl:value-of select="rows/content/form/label/new_class" disable-output-escaping="yes"/>
                        </label>
                        <select name="{rows/content/form/list/new_class/@name}" chosen="select" data-placeholder="{rows/content/form/list/new_class/option[position() = 1]}" class="select IGRP_change">
                          <xsl:call-template name="FIELD_validator">
                            <xsl:with-param name="field" select="rows/content/form/label/new_class"/>
                          </xsl:call-template>
                          <option value=""/>
                          <xsl:for-each select="rows/content/form/list/new_class/option[position() != 1]">
                            <option value="{value}">
                              <xsl:if test="@selected='true'">
                                <xsl:attribute name="selected">selected</xsl:attribute>
                              </xsl:if>
                ...
                <xsl:if test="rows/content/form/label/new_acti_name">
                  <xsl:if test="rows/content/form/label/new_acti_name">
                    <div class="col-1-4 item" item-name="new_acti_name">
                      <div class="igrp_item text">
                        <label>
                          <xsl:value-of select="rows/content/form/label/new_acti_name" disable-output-escaping="yes"/>
                        </label>
                        <input type="text" name="{rows/content/form/value/new_acti_name/@name}" value="{rows/content/form/value/new_acti_name}" class="text" maxlength="100">
                          <xsl:call-template name="FIELD_validator">
                            <xsl:with-param name="field" select="rows/content/form/label/new_acti_name"/>
                          </xsl:call-template>
                        </input>
                        <div class="_clear"/>
                      </div>
                ...
                <xsl:if test="rows/content/form/label/etapa_dest_dec">
                  <xsl:if test="rows/content/form/label/etapa_dest_dec">
                    <div class="col-1-4 item" item-name="etapa_dest_dec">
                      <div class="igrp_item text">
                        <label>
                          <xsl:value-of select="rows/content/form/label/etapa_dest_dec" disable-output-escaping="yes"/>
                        </label>
                        <input type="text" name="{rows/content/form/value/etapa_dest_dec/@name}" value="{rows/content/form/value/etapa_dest_dec}" class="text" maxlength="150">
                          <xsl:call-template name="FIELD_validator">
                            <xsl:with-param name="field" select="rows/content/form/label/etapa_dest_dec"/>
                          </xsl:call-template>
                        </input>
                        <div class="_clear"/>
                      </div>
