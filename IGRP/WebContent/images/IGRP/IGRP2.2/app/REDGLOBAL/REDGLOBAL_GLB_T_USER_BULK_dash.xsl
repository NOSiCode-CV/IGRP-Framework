            ...
            <form action="#" method="post" id="formular_default" name="formular_default" class="default_filter"><!-- SHOW HIDDEN--><xsl:apply-templates mode="form-hidden" select="rows/content/filter/value"/>
              <xsl:apply-templates mode="form-hidden" select="rows/content/form/value"/><!-- START FILTER--><div class="box-content resetPadding">
                <div class="col">
                  ...
                  <xsl:if test="rows/content/filter/label/org_fk">
                    <div class="col-1-4 item" item-name="org_fk">
                      <div class="igrp_item selectchange">
                        ...
                        <select name="{rows/content/filter/list/org_fk/@name}" chosen="select" data-placeholder="{rows/content/filter/list/org_fk/option[position() = 1]}" class="selectchange IGRP_change">
                          <xsl:call-template name="FIELD_validator">
                  ...
                  <xsl:if test="rows/content/filter/label/prof_type_fk">
                    <div class="col-1-4 item" item-name="prof_type_fk">
                      <div class="igrp_item select">
                        <label>
                          <xsl:if test="string-length(rows/content/filter/label/prof_type_fk) > 45">
                            <xsl:attribute name="title">
                              <xsl:value-of select="rows/content/filter/label/prof_type_fk" disable-output-escaping="yes"/>
                            </xsl:attribute>
                          </xsl:if>
                          <xsl:value-of select="rows/content/filter/label/prof_type_fk" disable-output-escaping="yes"/>
                        </label>
                        <select name="{rows/content/filter/list/prof_type_fk/@name}" chosen="select" data-placeholder="{rows/content/filter/list/prof_type_fk/option[position() = 1]}" class="select">
                          <xsl:call-template name="FIELD_validator">
                            <xsl:with-param name="field" select="rows/content/filter/label/prof_type_fk"/>
                          </xsl:call-template>
                          <option value=""/>
                          <xsl:for-each select="rows/content/filter/list/prof_type_fk/option[position() != 1]">
                            <option value="{value}">
                              <xsl:if test="@selected='true'">
                                <xsl:attribute name="selected">selected</xsl:attribute>
                              </xsl:if>
                  ...
                  <xsl:if test="rows/content/filter/label/email">
                    <div class="col-1-4 item" item-name="email">
                      <div class="igrp_item email">
                        <label>
                          <xsl:if test="string-length(rows/content/filter/label/email) > 45">
                            <xsl:attribute name="title">
                    ...
                    <table class="IGRP_table default-table" data-control="table">
                      <thead>
                        <tr>
                          ...
                          <xsl:if test="rows/content/table/label/profile">
                            <th align="left">
                              <xsl:value-of select="rows/content/table/label/profile"/>
                            </th>
                          </xsl:if>
                        </tr>
                      </thead>
                      <tbody>
                        <xsl:for-each select="rows/content/table/value/row[not(@total='yes')]">
                          <tr row="{position()}" class="IGRP_contextMenu">
                            ...
                            <xsl:if test="profile">
                              <td align="left" data-row="{position()}" data-title="{../../label/profile}" class="text">
