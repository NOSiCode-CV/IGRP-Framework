<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
            ...
            "linkDataArray":[<xsl:for-each select="rows/content/form/table/transition/value/row[not(@total='yes')]">
                                <xsl:choose>
                                    <xsl:when test="position() != last()">{
                                        ...
                                        "visible":<xsl:value-of select="visible"/></xsl:if><xsl:if test="textport and textport != ''">, 
                                        "text":"<xsl:value-of select="textport"/>"</xsl:if>},
                                    </xsl:when>
                                    <xsl:otherwise>{
                                        "from":<xsl:value-of select="from"/>, 
                                        "to":<xsl:value-of select="to"/>, 
              ...
              <!-- END FORM
              <input type="hidden" name="{../../label/category/@name}_fk" value="{category}" id="category" maxlength="30" rel="F_activity"/>
              <input type="hidden" name="{../../label/figure/@name}_fk" value="{figure}" id="figure"  maxlength="30" rel="F_activity"/>
              <input type="hidden" name="{../../label/text/@name}_fk" value="{text}" id="text" maxlength="30" rel="F_activity"/>
              <input type="hidden" name="{../../label/key/@name}_fk" value="{key}" id="key" maxlength="30" rel="F_activity"/>
              <input type="hidden" name="{../../label/loc/@name}_fk" value="{loc}" id="loc" maxlength="30" rel="F_activity"/>
              
              <input type="hidden" name="{../../label/to/@name}_fk" value="{to}" id="to"  maxlength="30" rel="F_transition"/>
              <input type="hidden" name="{../../label/from/@name}_fk" value="{from}" id="from"  maxlength="30" rel="F_transition"/>
              <input type="hidden" name="{../../label/fromport/@name}_fk" value="{fromport}" id="fromport" maxlength="30" rel="F_transition"/>
              <input type="hidden" name="{../../label/toport/@name}_fk" value="{toport}" id="toport" maxlength="30" rel="F_transition"/>
              <input type="hidden" name="{../../label/visible/@name}_fk" value="{visible}"  id="visible"  maxlength="30" rel="F_transition"/>
              <input type="hidden" name="{../../label/textport/@name}_fk" value="{textport}" id="textport" maxlength="30" rel="F_transition"/>-->
            </form>
  ...
  <xsl:include href="../../xsl/tmpl/IGRP-home-include.tmpl.xsl?v=20140415"/>
  <xsl:include href="../../xsl/tmpl/IGRP-JSvalidation.tmpl.xsl?v=20140415"/>
  <xsl:include href="../../xsl/tmpl/IGRP-field-validator.tmpl.xsl?v=20140415"/>
</xsl:stylesheet>
