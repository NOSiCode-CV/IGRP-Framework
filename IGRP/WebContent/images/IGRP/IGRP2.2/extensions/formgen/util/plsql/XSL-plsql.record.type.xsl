...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="name()='item' and (@type='specific' or @type='form' or @type='' or not(@type))">
                        <xsl:variable name="button_type">
                            <xsl:choose>
                                <xsl:when test="local-name(..)='group' or local-name(..)='tools-bar'">
    ...
    <xsl:template name="genJSVariables">
        ...
        <xsl:for-each select="$fields">
            ...
            <xsl:if test="@type='LOOKUP' or @type='lookup'">
            ...
            <xsl:value-of select="translate(translate(@type,'LOOKUP_',''),'lookup_','')" />
            <xsl:value-of select="$quotes" />
            <xsl:value-of select="$endline" />
        </xsl:for-each>
    </xsl:template>
