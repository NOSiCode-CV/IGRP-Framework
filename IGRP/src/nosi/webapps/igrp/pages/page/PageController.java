	...
	public Response actionGravar() throws IOException, IllegalArgumentException, IllegalAccessException{
		...
		if (Igrp.getInstance().getRequest().getMethod().toUpperCase().equals("POST")) {
			...
			if (idPage != 0) {
				...
				if (action != null)
					Igrp.getInstance().getFlashMessage().addMessage(FlashMessage.SUCCESS, gt("P�gina atualizada com sucesso."));
				else
			...
			if(fileJson!=null && fileXml!=null && fileXsl!=null && fileModel!=null && fileView!=null && fileController!=null && path_xsl!=null && path_xsl!=""  && path_class!=null && path_class!=""){
				...
				if(
					...
					if(error.equals("") || error==null){//Check if not error on the compilation class
						error = new Gson().toJson(new MapErrorCompile("Compila��o efetuada com sucesso", null));
						if (FileHelper.fileExists(Config.getWorkspace())) {
