...
public class PageController extends Controller {		
	...
	public Response actionListDomains() throws IOException{
		...
		resp.setContentType(Response.FORMAT_JSON);
		resp.setContent(json);
...
//				model.setP_self_id(a.getSelf_id());
	...
	public Response actionSaveGenPage() throws IOException, ServletException{		
		...
		if(ac!=null){			
			...
			if(fileJson!=null && fileXml!=null && fileXsl!=null && javaCode!=null && javaCode!="" && path_xsl!=null && path_xsl!=""  && path_class!=null && path_class!=""){
				...
				if(
					...
					ac.update();
					resp.setContent("<messages><message type=\"success\">Operacao efectuada com sucesso</message></messages>");
					return resp;
		...
		resp.setContent("<messages><message type=\"error\">Operacao falhada</message></messages>");
		return resp;//Igrp.getInstance().getResponse().getWriter().append();
	}
	...
	public Response actionPreserveUrl() throws IOException{
		...
		Response resp = new Response();
		resp.setContent("<your_code>"+your_code+"</your_code>");
		resp.setContentType(Response.FORMAT_XML);
		resp.setCharacterEncoding(Response.CHARSET_UTF_8);
   		resp.setType(1);
		return resp;
