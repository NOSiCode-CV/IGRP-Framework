...
public class PageController extends Controller {		
	...
	public Response actionEditar(@RParam(rParamName = "id")String id) throws IOException, IllegalArgumentException, IllegalAccessException{
		...
		if(Igrp.getInstance().getRequest().getMethod().equals("POST")){
			...
			if(!nosi.core.gui.page.Page.validatePage(action.getPage())){
				Igrp.getInstance().getFlashMessage().addMessage(FlashMessage.WARNING,"Nome de pÃ¡gina invÃ¡lida");
				return this.redirect("igrp", "page", "index", new String[]{"id"}, new String[]{action.getId() + ""});
			}
			action.setPage_descr(model.getAction_descr());
			action = action.update();
			if(action!=null)
				Igrp.getInstance().getFlashMessage().addMessage(FlashMessage.SUCCESS, "PÃ¡gina atualizada com sucesso.");
			else
				Igrp.getInstance().getFlashMessage().addMessage(FlashMessage.ERROR, "Error ao atualizar a pÃ¡gina.");
	...
	public Response actionVisualizar() throws IOException{
		...
		Action ac = new Action().findOne(Integer.parseInt(p_id));	
		if(ac!=null){			
			String content = FileHelper.readFileFromServer(Config.getResolvePathXsl(ac.getApplication().getDad(), ac.getPage(), ac.getVersion()),ac.getPage()+".xml");
