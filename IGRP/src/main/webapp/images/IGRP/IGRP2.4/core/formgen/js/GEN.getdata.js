...
var DATA = {
	get:function(p){
		...
		switch(p.type){
			...
			case 'color':
				rtn ='rgba(' + (Math.floor(Math.random() * 256)) + ',' + (Math.floor(Math.random() * 256)) + ',' + (Math.floor(Math.random() * 256)) + ',1)';
			...
			case 'img':
				...
				if(p && p.field ){
					if(p.field?.parent?.GET.type() == 'table')
						rtn = 'https://placehold.co/40x40';
					else if(p.field.GET.img)
						rtn = p.field.GET.img()
