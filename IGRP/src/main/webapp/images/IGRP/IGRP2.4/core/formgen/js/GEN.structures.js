var GENSTRUCTURES = function(GEN){
		...
		if(container.contextMenu.items[0]){
			...
			ctx.forEach(function(item){
				...
				var app    = item.action && item.action.app    ? item.action.app    : '',
					...
					_class = item.GET.class && item.GET.class() ? item.GET.class() : 'light',
					btnStyle = item.GET.btnStyle ? item.GET.btnStyle() : '',
					iconPosition = item.GET.iconPosition ? item.GET.iconPosition() : 'left',
	...
	var genItems = function(fields,rel){
		...
		fields.forEach(function(f){
			...
			var tag    = f.GET.tag(),
				...
				btnStyle = f.GET.btnStyle ? f.GET.btnStyle() : '',
	...
	var getXMLOptions = function(f){
		...
		var rtn = '<list>';
		var placeholder = f.GET.placeholder && f.GET.placeholder()? f.GET.placeholder() : '--';
		
		if(f.type == 'select')
			rtn+='<option><text>'+placeholder+'</text><value></value></option>';
