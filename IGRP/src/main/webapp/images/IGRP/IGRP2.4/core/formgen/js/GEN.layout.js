var GEN_LAYOUT = function(viewer){
	...
	layout.addRow = function(p){
		...
		if(view[0]){
			...
			const StructureDropDown = row.find('[rel="columns"] .add-columns')[0];

			StructureDropDown?.addEventListener('show.bs.dropdown', function () {
				const selectedStruc = row.attr('layout') || '12';
				row.find('.column-setter').removeClass('active');
	...
	var setEvents = function(){
			...
			if(layoutStr == 'custom'){
				...
				$.each(row.find('>'+VARS.layout.columns),function(i,col){
					...
					if (customClass){
						...
						customCol += customClass[1].split('col-md-')[1];
						idx +=1;
					}
				});
		...
		//add Row
		$(VARS.html.view).on('click',VARS.layout.addRow+',.add-row-control',function(){
