var CONTAINER = function(name,params){
	...
	container.propertiesLabels= {
		title : $.IGRP.locale.get('title'),
		hasTitle : $.IGRP.locale.get('gen-show-title'),
	...
	container.getXSLIncludes = function(relative){
		...
		var _setinclude = function(name){
			...
			href = name.path ? '/includes/xsl/'+name.path : name+'.xsl'; 
	...
	var afterTransform = function(contents){
		...
		$(function(){
			...
			if(container.xslOptions.useDefault){
				...
				var fffix = container.holder.is('.disable-output-escaping') ? container.holder : container.holder.find('.disable-output-escaping');
	...
	var setUpNoField = function(area,o){
		if(o){
			...
			area.append('<div class="gen-no-field-info">DROP <a href="javascript:void(0)" class="gen-activate-fields-tab"><b> FIELDS</b></a> OR <a href="#" class="gen-copy-from"><b> COPY</b></a></div>')
		}else{
	...
	container.reorderFieldsByElements = (fieldsHolder)=>{
		...
		if(fieldsHolderLength == FIELDS.length){
			...
			if(container.groups){
				...
				container.groups.items.forEach(function(g){
					var groupId = g.id;
					$.each(fieldsHolder,function(pos,fieldHolder){
...
*/				
	...
	container.GET = {
		...
		propertyFields : function(){
			...
			for(var p in container.propertiesOptions){
				...
				var prop = container.propertiesOptions[p];
	...
	container.validField = function(name){
		...
		var valid = container.canRecieveFields ? true : false;
	...
	var setVars = function(){//set container global vars
		...
		container.id     = params.id || container.type+(new Date()).getTime();
		...
		container.acceptableFields = function(){
			const acceptArray = []
			$('#gen-list-fields .gen-fields>li').each(function(i,f){
	...
	var setEvents = function(){
		...
		$( '.container-field-item ', container.holder ).on('click', function(){
			...
			if(f){
				...
				container.SET.fields([field],null,function(){
