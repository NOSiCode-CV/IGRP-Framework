...
';
  ...
  <xsl:template name="IGRP-topmenu">
    ...
    <xsl:attribute name="has-menu">
      <xsl:value-of select="$hasMenu"/>
    </xsl:attribute>
    ...
    <xsl:if test="$hasMenu = 'true'">
      ...
      <div class="col-md-2 col-sm-3 sidebar tree-list" id="igrp-sidebar">
        ...
        <div class="igrp-sidebar-menu-search-wrapper">
          <input type="text" class="form-control igrp-search-list" placeholder="Pesquisar Menu" search-list="#igrp-sidebar .treeview>a, #igrp-sidebar ul.treeview-menu>li>a" search-attr="text" search-item-parent=".treeview" search-item-wrapper=".treeview-menu" />
        </div>
    ...
    <div id="igrp-global-modal" class="modal fade" data-backdrop="static" tabindex="-1" role="dialog">
      ...
      <div style="display:none!important">
        <text id="confirm-text">
          <xsl:value-of select="rows/content/messages/message[@type='confirm']"/>
        </text>
      </div>
    </div>
    <!--/GLOBAL MODAL -->
              ...
              <div class="igrp-debug-contents clearfix">
                ...
                <ul class="clear igrp-debug-list">
                  <xsl:for-each select="rows/content/messages/message[@type='debug']">
                    <li style="white-space: pre-wrap" value="{.}">
                      <xsl:value-of select="." />
                    </li>
                  </xsl:for-each>
                </ul>
  ...
  <xsl:template name="igrp-messages" match="messages" mode="igrp-messages">
    ...
    <xsl:variable name="dismissClass">
      <xsl:if test="$dismiss = 'true' ">
        <xsl:value-of select="alert-dismissible"/>
      </xsl:if>
    </xsl:variable>
    <!--<xsl:if test="message">-->
...
!important;
          color: <xsl:value-of select="@bg-text-color"/>
!important;
        }
        .cp-<xsl:value-of select="@name"/>
-text {
...
<xsl:for-each select="colors/color"> [bg-color="<xsl:value-of select="@name"/>
"]{
...
!important;
            ...
            } [active-bg-color="<xsl:value-of select="@name"/>
"].active{
              background-color:<xsl:value-of select="."/>
!important;
<xsl:if test="@text-color">
...
<xsl:if test="@bg-hover"> [bg-hover="<xsl:value-of select="@name"/>
"]:hover{
                background-color:<xsl:value-of select="@bg-hover"/>
;
              }
</xsl:if> [border-color="<xsl:value-of select="@name"/>
"]{
...
"]{
              color:<xsl:value-of select="."/>
;
            } [active-text-color="<xsl:value-of select="@name"/>
"].active,
...
<xsl:if test="colors/color[@name='primary']">
            .checkbox-switch input:checked + .slider{
              background-color: <xsl:value-of select="colors/color[@name='primary']"/>
!important;
            }
</xsl:if>
...
</xsl:variable> [active-text-color="primary"].active,
        .active [active-text-color="primary"]{
            color:<xsl:value-of select="$mainColor"/>
!important;
        }

<xsl:if test="not(colors/color)"> [bg-color="primary"]{
            background-color:<xsl:value-of select="$mainColor"/>
!important;
...
!important;
            ...
            border-color:<xsl:value-of select="$mainColor"/>
!important;
          }
...
<xsl:variable name="secondaryColor" select="'#5b5b5b'"/>
 [bg-color="secondary"]{
            background-color:<xsl:value-of select="$secondaryColor"/>
!important;
...
!important;
            ...
            border-color:<xsl:value-of select="$secondaryColor"/>
!important;
          }
...
!important;
        ...
        #igrp-form-gen #igrp-sidebar .nav-tabs > li.active > a:after{
          background:<xsl:value-of select="$mainColor"/>
!important;
        }*/
...
<li>
<a href="#">
<span class="igrp-theme-color" color="{.}" text-color="{@bg-text-color}" style="background:{.}"></span>
</a>
</li>
</xsl:for-each>
<!-- <li>
