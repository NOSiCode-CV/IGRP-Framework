const GENSTRUCTURES = function (GEN) {
    const genFields = function (container) {
        let rtn = '<fields>';
        const fields = container.GET.fields();
        const containerTag = container.GET.tag();
        ...
        if (container.GET.filter && container.GET.filter()) {
            const filter = $.trim(container.GET.filter());
            if (filter) {
        ...
        for (let p in container.propertiesOptions) {
            const pr = container.propertiesOptions[p];

            if (pr.isField) {
                const tag = containerTag + '_' + pr.name;
                const persist = pr.valuePersist ? 'persist="true"' : '';
                const valueObj = pr.value;
                const isAction = typeof valueObj == 'object' && valueObj.type === 'action';
                let value = isAction ? valueObj : container.GET[pr.name]();
                //var value = container.GET[pr.name]();
                let valueAttr = '';
                const label = pr.label || '';
                const fieldType = isAction ? 'link' : 'text';
	...
	const genMap = function (container) {
const subpath = path.substring(1);
return '..' + subpath.substring(subpath.indexOf('/')) + '/xml/gis/map.xml';
};
	const genTable = function (container) {
        const fields = container.GET.fields();
        const atts = "";

        const genTableFields = function (o) {

            let rtn = '';
            const total = o ? o.total : false;

            fields.forEach(function (f) {

                const tag = f.xml.tag ? f.xml.tag : f.GET.tag();
                ...
                let attributesStr = getAttrsArr(f, ['name']);

                const visible = f.GET.visible && !f.GET.visible() ? ' visible="false"' : '';
        ...
        if (container.xml.tableLegend) {
            ...
            const lg = container.xml.getLegendColors && container.xml.getLegendColors() ? container.xml.getLegendColors() : '';
	...
	const getAttrsArr = function (field, which) {
        ...
        if (which && which[0])
            ...
            for (const name in field.proprieties) {
	...
	const genLookUpField = function (f) {

        console.log(f);

        const link = (f.action && f.action.link) ? '..' + f.action.link.substring(1).substring(f.action.link.substring(1).indexOf('/')) : '';
        let rtn = '<lookup>' + link + '</lookup>';
        const isTable = f.parent.GET.type() === 'formlist' ? 'is-table="true"' : '';
        ...
        if (f.GET.lookupParams) {
            ...
            f.GET.lookupParams().forEach(function (lp) {
                ...
                const target = lp.name || lp.target_field,
	...
	const getXMLOptions = function (f) {
        ...
        if (f.GET.domain && f.GET.domain() && f.proprieties.domain_value && f.proprieties.domain_value[0]) {
            ...
            f.proprieties.domain_value.forEach(function (d) {
                const value = $.IGRP.utils.htmlEncode(d.value),
	...
	const checkRules = function (f, sxml) {
        ...
        if (rules && rules[0]) {
            ...
            rules.forEach(function (r) {
                ...
                const actions = r.actions ? JSON.parse(r.actions.replace(/'/g, '"')) : [];

                actions.forEach(function (a) {

                    const action = a.gen_rule_action;
                    ...
                    switch (action) {
                        ...
                        case 'remote_list':
                            ...
                            const proc = a.gen_rule_procedure,
                                ...
                                reqFields = function () {
                                    ...
                                    if (requestFields) {
                                        const farr = requestFields.split(',');
...
//										});
	...
	const genContainerStruc = function (p) {
        let rtn = "";
        const container = p.object;

        const tag = $.trim(container.GET.tag()),
            ...
            typeAttr = type;
        let attributes = ' type="' + type + '" xml-type="' + xmlType + '"';

        if (container.copyOptions) {
            //console.log(container.copyOptions)
            const cpackageDB = container.copyOptions.plsql ? container.copyOptions.plsql.package : '',
            ...
            if (container.xml.getStructure) {
                ...
                if (container.xml.gen) {
                    ...
                    if (container.xml.type === 'text' || container.xml.type === 'texteditor') {
                        ...
                        const value = container.GET.text ? container.GET.text() : '',
    ...
    const fieldXMLStructute = function () {
    ...
    var genFieldStruc = function (p) {
        let rtn = "";
        const field = p.field;
        const tag = field.xml.tag ? field.xml.tag : field.GET.tag(),
            type = field.GET.type(),
        ...
        if (tag) {
            ...
            const eventResponse = $(document).triggerHandler("gen-field-structure", [field]);
	...
	GEN.genContextMenu = function (container) {
		...
		if (container.contextMenu.items[0]) {
            const ctx = container.contextMenu.items;
            const tag = container.contextMenu.xmlTag ? container.contextMenu.xmlTag : 'context-menu';
            const itemType = container.GET.type() === 'form' ? 'form' : 'specific';
			rtn = '<' + tag + '>';
			const targetConfirm = ['confirm', 'alert_submit'];

			ctx.forEach(function (item) {

                const app = item.action && item.action.app ? item.action.app : '',
                ...
                if (item.GET.refresh_components && item.GET.refresh_components()) {
                    ...
                    const comps = item.GET.refresh_components();

                    refresh_components = comps.join(',');

                }

                const linkAction = (item.action && item.action.link) ? '..' + item.action.link.substring(1).substring(item.action.link.substring(1).indexOf('/')) : '';
    ...
    var genItems = function (fields, rel) {
        ...
        fields.forEach(function (f) {
            ...
            const tag = f.GET.tag(),
                title = f.GET.label();
            let target = f.GET.target ? f.GET.target() : "";
            const img = f.GET.img ? f.GET.img() : "",
                ...
                customReturnAttr = customReturn ? 'custom_return="true"' : '';
            let refresh_components = '';
            const adbcli = target === 'sharpadbclient' ? 'sharpadbclient="' + f.GET.adbcli() + '"' : '',
                labelConfirm = targetConfirm.includes(target) ? 'labelConfirm="Deseja realmente realizar esta operação?"' : '';
            ...
            if (f.GET.refresh_components && f.GET.refresh_components()) {
                ...
                const comps = f.GET.refresh_components();
    ...
    var genGraphic = function (container) {
        const colors = '<colors>' +
            ...
            '<col>' + DATA.get({type: 'color', format: 'hex'}) + '</col>' +
            '</colors>';
        const chartType = container.GET.chart_type ? container.GET.chart_type() : 'line';
    ...
    const getFieldsService = function (service, point) {
        let rtn = '';
        service.forEach(function (c) {
            let type = c.type ? 'type="' + c.type + '"' : '';
            if (c[0]) {
                const node = c[0],
    ...
    GEN.getFieldServiceMap = function (service) {
        const GEN = VARS.getGen(),
            _package = service.package ? 'package="' + service.package + '"' : '';
        let rtn = '<service code="' + service.code + '" proc="' + service.proc + '">';
        rtn += '<request>';
        const serviceReq = service.fieldsReq[0] ? service.fieldsReq : service.fieldsRes;
        rtn += getFieldsService(service.fieldsReq, 'from');
        rtn += '</request>';

        rtn += '<response ' + _package + '>';
        const serviceRes = service.fieldsRes[0] ? service.fieldsRes : service.fieldsReq;
	...
	const returnAttr = function (field, name) {
        let arr = '';
        if (field.GET[name]) {
    ...
    const validAttrArr = function (name) {
    ...
    const getAttrStr = function (field, name) {
