package cv.nosi.webapps.igrp_studio.pages.modulo;
...
public class ModuloController extends Controller {
	public Response actionIndex() throws IOException, IllegalArgumentException, IllegalAccessException{
		...
		if(Core.isNotNull(p_modulo_cod)) {
			cv.nosi.webapps.igrp.dao.Modulo m = new cv.nosi.webapps.igrp.dao.Modulo().find().where("name","=",p_modulo_cod).one();
			if(Core.isNotNull(m)) {
	...
	public Response actionGravar() throws IOException, IllegalArgumentException, IllegalAccessException{
      ...
      try {
			...
			if(Core.isNotNull(p_modulo_cod)) {
				...
				cv.nosi.webapps.igrp.dao.Modulo m = new cv.nosi.webapps.igrp.dao.Modulo();
				...
				if (new cv.nosi.webapps.igrp.dao.Modulo().find().andWhere("name", "=", model.getModulo()).andWhere("application", "=", Core.toInt(model.getAplicacao())).one() == null) {
					m = m.insert();
					if(m == null) {
						Core.setMessageError();
						return forward("igrp_studio","Modulo","index", this.queryString());
