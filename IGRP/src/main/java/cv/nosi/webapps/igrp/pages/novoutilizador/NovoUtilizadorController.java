package cv.nosi.webapps.igrp.pages.novoutilizador;
...
import cv.nosi.core.exception.HttpException;
import cv.nosi.core.exception.ServerErrorHttpException;
import cv.nosi.core.ldap.LdapPerson;
import cv.nosi.core.mail.EmailMessage.PdexTemplate;
...
import cv.nosi.core.webapp.workflow.activit.rest.entities.UserService;
import cv.nosi.core.webapp.workflow.activit.rest.services.GroupServiceRest;
import cv.nosi.core.webapp.workflow.activit.rest.services.UserServiceRest;
...
//			The user is not activated because the email send is to activate/confirm the account
				...
				userLdap.setAuth_key(cv.nosi.core.webapp.component.User.generateAuthenticationKey());
				userLdap.setActivation_key(cv.nosi.core.webapp.component.User.generateActivationKey());
			}
		} else
	...
	private boolean addRoleToUser(User user) {
		...
		try {
			...
			cv.nosi.core.webservices.soap.client.SoapClient sc = Core.soapClient(wsdlUrl, namespaces, headers,
					bodyContent);
...
//								ENV not added, so must be inserted the application
	...
	public static void main(String[] args) { 
	       ...
	       // headers.put("SOAPAction", "\"http://ws.cdyne.com/ResolveIP\""); 
	        ...
	        cv.nosi.core.webservices.soap.client.SoapClient sc = Core.soapClient(
