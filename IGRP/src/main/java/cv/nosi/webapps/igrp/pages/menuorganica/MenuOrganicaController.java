...
public class MenuOrganicaController extends Controller {
	public Response actionIndex() throws IOException, IllegalArgumentException, IllegalAccessException{
		...
		if (Core.isNotNullOrZero(env_fk))
			model.setEnv_fk(env_fk);
		else
			env_fk = model.getEnv_fk();
		...
		if (model.getType().equals("org")) {
			view.btn_novo.setLink("igrp", "MenuOrganica", "novo");
			view.btn_novo.addParameter("env_fk",env_fk);
			view.btn_novo.setVisible(true);
		}
		view.btn_gravar.addParameter("env_fk",env_fk);
		if (model.getType().equals("user") && user != null && profile != null) {
			view.btn_gravar.addParameter("user_id", user.getId()).addParameter("userEmail", user.getEmail())
...
/*----#start-code(custom_actions)----*/
	...
	private void removeMenu(MenuOrganica model, List<String> uncheckedIds) {
		if (model.getType().equals("org")) {
			...
			this.deleteMenu(uncheckedIds, "MEN_USER", Core.getParamInt("org_id"), Core.getParamInt("prof_id"),
					Core.getParamInt("user_id"));
		}
	}

	private void deleteMenu(List<String> uncheckedIds, String type, int org_id, int prof_id, int user_id) {
		for (String m : uncheckedIds) {
			ResultSet r = Core.delete(ConfigDBIGRP.FILE_NAME_HIBERNATE_IGRP_CONFIG, "tbl_profile").where()
					.andWhere("type", "=", type).andWhere("type_fk", "=", Core.toInt(m))
					.andWhere("prof_type_fk", "=", prof_id).andWhere("user_fk", "=", user_id)
					.andWhere("org_fk", "=", org_id).execute();
	...
	private void insetMenu(MenuOrganica model, List<String> chekedIds) {
		...
		if (model.getType().equals("org")) {
			...
			ProfileType pt = new ProfileType().findOne(model.getId());
			success = this.insertMenu(chekedIds, "MEN", pt.getOrganization().getId(), pt.getId(),
					this.userAdmin.getId());

		} else if (model.getType().equals("user")) {
			success = this.insertMenu(chekedIds, "MEN_USER", Core.getParamInt("org_id"), Core.getParamInt("prof_id"),
					Core.getParamInt("user_id"));
		}
		if (success)
	...
	private boolean insertMenu(List<String> chekedIds, String type, int org_id, int prof_id, int user_id) {
		...
		for (String m : chekedIds) {
			ResultSet r = Core.insert(ConfigDBIGRP.FILE_NAME_HIBERNATE_IGRP_CONFIG, "tbl_profile")
					.addString("type", type).addInt("type_fk", Core.toInt(m)).addInt("prof_type_fk", prof_id)
					.addInt("user_fk", user_id).addInt("org_fk", org_id).execute();
			if (r != null && r.hasError()) {
				Core.setMessageError(r.getError());
				success = false;
				break;
