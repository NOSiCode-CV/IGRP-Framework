package cv.nosi.webapps.igrp.pages.page;
...
import cv.nosi.core.cversion.git.GitClient;
import cv.nosi.core.webapp.Igrp;
import cv.nosi.core.webapp.Response;
import cv.nosi.core.webapp.compiler.helpers.Compiler;
import cv.nosi.core.webapp.compiler.helpers.MapErrorCompile;
import cv.nosi.core.webapp.mvc.Controller;
import cv.nosi.core.webapp.util.Core;
import cv.nosi.core.webapp.util.FlashMessage;
import cv.nosi.core.webapp.util.helpers.ExtractReserveCode;
import cv.nosi.core.webapp.util.helpers.IgrpHelper;
import cv.nosi.core.webapp.util.helpers.Route;
import cv.nosi.core.webapp.util.helpers.database.QueryHelper;
...
import cv.nosi.webapps.igrp.dao.Share;
import cv.nosi.webapps.igrp.dao.Transaction;
import cv.nosi.webapps.igrp.pages.dominio.DomainHeper;
	...
	public Response actionGravar() throws IOException, IllegalArgumentException, IllegalAccessException{
		...
		/*----#start-code(gravar)----*/
			...
			if (idPage != 0) {
				...
				action.setStatus(model.getStatus());
				action.setTipo((short)model.getPublico());
				action.setPage(cv.nosi.core.gui.page.Page.getPageName(model.getPage()));
				...
				action.setXsl_src(action.getApplication().getDad().toLowerCase() + "/" + action.getPage().toLowerCase()
						+ "/" + action.getPage() + ".xsl");
				if (!cv.nosi.core.gui.page.Page.validatePage(action.getPage())) {
 ...
 * */
	...
	private boolean checkifexists(Page model) {
		...
		return Core.isNull(new Action().find().andWhere("application.id", "=", Core.toInt(model.getEnv_fk()))
				.andWhere("page", "=", cv.nosi.core.gui.page.Page.getPageName(model.getPage())).one());
		...
		if(conf.getEnvironment() != null && conf.getEnvironment().equalsIgnoreCase("dev")) {
			...
			try {
				...
				if(isJavaOk && isXslOk) {
					...
					try {
						gitUri = new cv.nosi.webapps.igrp.dao.Config().find().andWhere("name", "=", dad + ".git.uri").one().getValue(); 
						username = new cv.nosi.webapps.igrp.dao.Config().find().andWhere("name", "=", dad + ".git.username").one().getValue();
						password = new cv.nosi.webapps.igrp.dao.Config().find().andWhere("name", "=", dad + ".git.password").one().getValue();
					} catch (Exception e) {
						e.printStackTrace(); 
					}
