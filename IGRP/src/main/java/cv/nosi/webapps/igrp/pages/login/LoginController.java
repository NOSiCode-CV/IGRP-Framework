package cv.nosi.webapps.igrp.pages.login;
...
import cv.nosi.core.ldap.NosiLdapAPI;
...
import cv.nosi.webapps.igrp.dao.Session;
...
public class LoginController extends Controller {
	...
	private boolean loginWithDb(String username, String password) { 
		...
		if (user != null && user.validate(cv.nosi.core.webapp.component.User.encryptToHash(username + "" + password, "SHA-256")) && userIsAuthenticatedFlag(user)) {
			if (user.getStatus() == 1) {
		...
		if (success) {
			...
			if (user != null) {
				...
				if (this.getConfig().getEnvironment().equals(ConfigCommonMainConstants.IGRP_ENV_DEV.value())) { // auto invite 
					...
					newUser.setAuth_key(cv.nosi.core.webapp.component.User.generateAuthenticationKey());
					newUser.setActivation_key(cv.nosi.core.webapp.component.User.generateActivationKey());
