...
import cv.nosi.core.xml.XMLApplicationReader;
import cv.nosi.core.xml.XMLPageReader;
	...
	private List<Action> savePage(FileImportAppOrPage file,Application app,String type){
		...
		try {
			...
			for(Action page : listPage.getRow()){
				...
				if(type.equals("java")){
					if(cv.nosi.core.gui.page.Page.validatePage(page.getPage()) && pageCheck==null){
					...
					if(Core.isNotNull(page.getPage())&& pageCheck ==null && cv.nosi.core.gui.page.Page.validatePage(page.getPage())&& page.getImg_src()==null){
