...
import cv.nosi.webapps.igrp_studio.pages.bpmndesigner.ReserveCodeControllerTask;
/**
 * Emanuel
 * 7 May 2018
 */
public final class BPMNHelper {
				...
				tipoDocsIN.stream().forEach(t->{
					 t.setFileId(-1);
		 			 ...
		 			 if(t.getTipoDocumento()!=null) {
						 cv.nosi.webapps.igrp.dao.TaskFile taskFile = new cv.nosi.webapps.igrp.dao.TaskFile().find()
	...
	private static List<TipoDocumentoEtapa> getDocumentOutputReport(String taskDad,String processDefinition,String taskDefinition) { 
		...
		for(TipoDocumentoEtapa t : tipoDocs) { 
			...
			cv.nosi.core.webapp.util.report.Report r = Core.getLinkReport(t.getRepTemplate().getCode()); 
