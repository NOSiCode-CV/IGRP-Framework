...
import cv.nosi.core.gui.components.IGRPFormList;
...
import cv.nosi.core.gui.fields.FileField;
import cv.nosi.core.gui.fields.HiddenField;
import cv.nosi.core.gui.fields.LinkField;
import cv.nosi.core.gui.fields.TextField;
import cv.nosi.core.webapp.util.Core;
import cv.nosi.core.webapp.workflow.bpmn.FormlistDocument.Formlist_documento_task;
...
public class DisplayDocmentType{		
	...
	private String displayInputDocs() {
		IGRPFormList formlist_documento_task = new IGRPFormList("formlist_documento_task", "Documentos de Input");
	...
	private String displayOutputDocs() {
		IGRPFormList formlist_documento_output_task = new IGRPFormList("formlist_documento_output_task", "Documentos de Output");
		formlist_documento_output_task.getProperties().put("type", "workflow_document");
	...
	public String displayInputNOutputDocsInDistinctFormList() {
		String xml = "";
		xml += displayInputDocs();
		xml += displayOutputDocs();
		return xml;
	}
	
	public String displayAllDocsInSameFormList() {
		String xml = "";
		xml += displayInputNOutputDocs();
		return xml;
	}
	...
	private void addField(IGRPFormList formlist_documento_task) { 
		...
		formlist_documento_id_tp_doc = new TextField(null, "formlist_documento_id_tp_doc");
		formlist_documento_id_tp_doc.propertie().add("name", "p_formlist_documento_id_tp_doc").add("tag","formlist_documento_id_tp_doc").add("type","hidden").add("maxlength","100").add("required","false").add("desc","true");  
	...
	private void addField_(IGRPFormList formlist_documento_task) {
		...
		formlist_documento_output_task_nome.setLabel(gt("Nome"));
		formlist_documento_output_task_nome.propertie().add("name","p_formlist_documento_output_task_nome").add("type","text").add("maxlength","100").add("required","false").add("desc","true");
		...
		formlist_documento_task.addField(formlist_documento_output_task_descricao);
		formlist_documento_task.addField(formlist_documento_output_task_obrigatoriedade);
		formlist_documento_task.addField(formlist_documento_output_user);
