...
import cv.nosi.base.ActiveRecord.HibernateHintOption;
...
public class QuerySelect extends CommonFIlter{
	...
	public ResultSet executeQuery(Config_env env) {
		return executeQuery(cv.nosi.core.webapp.util.helpers.database.Connection.getConnection(env));
	}
	
	public ResultSet executeQuery(String connectionName) {
		return executeQuery(cv.nosi.core.webapp.util.helpers.database.Connection.getConnection(connectionName));
	}
