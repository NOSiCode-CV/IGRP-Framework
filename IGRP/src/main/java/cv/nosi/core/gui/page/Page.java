...
import cv.nosi.core.exception.NotFoundHttpException;
import cv.nosi.core.gui.components.IGRPLogBar;
			...
			if(action.getParameterCount() > 0){
				for(Parameter parameter : action.getParameters()){
					if(parameter.getType().getSuperclass().getName().equals("nosi.core.webapp.Model")){
						...
						cv.nosi.core.webapp.mvc.Model model = (Model) c_.newInstance();
						model.load();
