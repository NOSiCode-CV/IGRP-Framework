...
public class LoginController extends Controller {
	...
	public Response actionLogout() throws IOException {
		...
		if (r != null && r.equalsIgnoreCase(ConfigCommonMainConstants.IGRP_AUTHENTICATION_TYPE_OAUTH2_OPENID.value())) {
			String oidcLogout = settings
					.getProperty(ConfigCommonMainConstants.IDS_OAUTH2_OPENID_ENDPOINT_LOGOUT.value());
			if (oidcLogout != null && !oidcLogout.isEmpty()) {
				...
				aux = redirect_uri != null && !redirect_uri.isEmpty()
						? aux + "&post_logout_redirect_uri=" + redirect_uri
						: aux;
	...
	private boolean authenticateDirectlyToLDAPServer(String username, String password,
			ArrayList<LdapPerson> personArray) {
	...
	private boolean authenticateThroughIdentityServerAutentika(String username, String password,
			List<LdapPerson> personArray) {
		boolean flag = false;
		...
		username = v != null && v.equalsIgnoreCase("true")
				? RemoteUserStoreManagerServiceConstants.GOVCV_TENANT + "/" + username
				: username;
	...
	private Map<String, String> oAuth2Wso2GetUserInfoByToken(String token) {
		...
		try {
			...
			Client curl = ClientBuilder.newClient();
			javax.ws.rs.core.Response r = curl.target(endpoint).request().header("Accept", "application/json")
					.header("Authorization", "Bearer " + token).get(javax.ws.rs.core.Response.class);
	...
	public Response oAuth2Wso2() {
		final String NIC_PATTERN = "^\\d{8}[MmFf]{1}\\d{3}[a-zA-Z]{1}$";
		...
		if (r != null && r.equalsIgnoreCase(ConfigCommonMainConstants.IGRP_AUTHENTICATION_TYPE_OAUTH2_OPENID.value())) {
				...
				if (_r != null && _r.containsKey("email") && _r.containsKey("sub")) {
					...
					if (user != null) {
						...
						if (new Config().getEnvironment()
								.equalsIgnoreCase(ConfigCommonMainConstants.IGRP_ENV_DEV.value())) {
							...
							try {
								...
								newUser = newUser.insert();
								this.afterLogin(user);
								if (newUser != null && createPerfilWhenAutoInvite(newUser)
										&& createSessionLdapAuthentication(newUser)) {
			...
			if ((error == null || error.isEmpty()) && (authCode == null || authCode.isEmpty())
	...
	private void afterLogin(User user) {
		...
		if (Core.isNotNull(dad) && !dad.equalsIgnoreCase("igrp") && !dad.equalsIgnoreCase("igrp_studio")
				&& !dad.equalsIgnoreCase("tutorial")) {
