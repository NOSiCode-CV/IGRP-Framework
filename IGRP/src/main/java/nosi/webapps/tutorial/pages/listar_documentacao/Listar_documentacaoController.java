...
public class Listar_documentacaoController extends Controller {
	public Response actionIndex() throws IOException, IllegalArgumentException, IllegalAccessException{
		Listar_documentacao model = new Listar_documentacao();
		model.load();
		Listar_documentacaoView view = new Listar_documentacaoView();
		/*----#start-code(index)----*/
		  try{
	
          List<Tipo> tipoList = new Tipo().find().all();

          List< Listar_documentacao.Treemenu_1> tipoTable = new ArrayList<>();
          
          if(tipoList != null){
        	  
              for(Tipo tip : tipoList){

                  Listar_documentacao.Treemenu_1 row = new Listar_documentacao.Treemenu_1();
	        ...
	        if (document!=null) {
	          ...
	          if(p_id2 != null){
	                ...
	                for(String x : p_id2){
	               	...
	               	IGRPButton btn_relaciona = new IGRPButton(documen.getTitulo(),"igrp_web_doc","Teste_treemenu","index&p_id="+documen.getIdDoc(),"_self","link|fa-angle-right","","");
...
public Response actionRemote_treemenu_1() throws IOException, IllegalArgumentException, IllegalAccessException{
        ...
        if(Core.isNotNullOrZero(p_id)){
            ...
            documentfilter.andWhere("tipo","=",Core.toInt(p_id));
        }
   ...
   public String getXml(String id, String idpai, String desc_menu, String child) {
	   ...
	   String xml = "<row>" + 
		"<treemenu_1_link>" + StringEscapeUtils.escapeXml11(Core.getIGRPLink("igrp_web_doc", "Teste_treemenu", "index")+"&p_id="+id)+"</treemenu_1_link>" +		
