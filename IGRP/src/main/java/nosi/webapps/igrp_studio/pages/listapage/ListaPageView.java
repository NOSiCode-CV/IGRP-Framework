...
public class ListaPageView extends View {
	...
	public ListaPageView(){
		...
		link_btn_nova_pagina.setValue("webapps?r=igrp_studio/ListaEnv/index");
		...
		crud_generator.setValue("webapps?r=igrp_studio/CRUDGenerator/index");
		...
		btn_import.setValue("webapps?r=undefined/undefined/undefined");
		...
		nome_page.propertie().add("name","p_nome_page").add("type","text").add("maxlength","30");
		
		p_id_page = new HiddenField(model,"p_id_page");
		p_id_page.setLabel(gt(""));
		p_id_page.propertie().add("name","p_id_page").add("type","hidden").add("maxlength","30").add("tag","id_page");
		...
		aplicacao.setValue("webapps?r=igrp_studio/ListaPage/index");
