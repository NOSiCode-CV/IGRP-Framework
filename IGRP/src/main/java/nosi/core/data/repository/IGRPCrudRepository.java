...
public abstract class IGRPCrudRepository<T, ID extends Serializable> implements IIGRPCrudRepository<T, ID> {
   ...
   @Override
   public IGRPUpdateQuery<T> updateQuery() {
      final CriteriaUpdate<T> criteriaUpdate = this.session.getCriteriaBuilder().createCriteriaUpdate(this.clazz);
      final Root<T> root = criteriaUpdate.from(this.clazz);
      return new IGRPUpdateQuery<>(this.session, this.clazz, criteriaUpdate, root);
   }
