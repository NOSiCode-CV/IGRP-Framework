...
public class Import {
	...
	protected Application saveApp(FileImportAppOrPage file, boolean isPlsql) {
		InputStream is = new ByteArrayInputStream(file.getConteudo().getBytes(isPlsql ? StandardCharsets.ISO_8859_1 : StandardCharsets.UTF_8));
	...
	private List<Action> savePage(FileImportAppOrPage file,Application app,String type){
		...
		InputStream is;
       final var isPlsql = type.equals("plsql");
       is = new ByteArrayInputStream(file.getConteudo().getBytes(isPlsql ?StandardCharsets.ISO_8859_1: StandardCharsets.UTF_8));
       ...
       for(Action page : listPage.getRow()){
          ...
          if(isPlsql){ //Se for de psql, assume Page como Action
          ...
          if(type.equals("java")){
             if(Page.validatePage(page.getPage()) && pageCheck == null){
          ...
          }else if(isPlsql){// Caso for de psql, valida nome de classe e versao da pagina
             if(Core.isNotNull(page.getPage())&& pageCheck ==null && Page.validatePage(page.getPage())&& page.getImg_src()==null){
